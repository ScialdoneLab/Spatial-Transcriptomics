...
#DV_dataset<-read.csv("./DV_dataset_G99.csv", stringsAsFactors = FALSE, sep="\t", row.names = 1, header=FALSE)

#colnames(DV_dataset)<-paste(DV_dataset[1,], 1:dim(DV_dataset)[2], sep="")
#DV_dataset<-DV_dataset[,-grep("0610005C13Rik", colnames(DV_dataset))]

#DV_names<-read.csv("./DVsample_names_G99.csv", header=FALSE, stringsAsFactors = FALSE)

#DV_names<-DV_names[,1]
#DV_names<-gsub(".counts.csv", "", DV_names)
#colnames(DV_dataset)<-DV_names

#write.csv(DV_dataset, "./DV_datasetNew_G99.csv")
```

```{r AssembleXIDataset, echo=FALSE, message=FALSE, warning=FALSE}

#XIDV1_dataset<-read.csv("./XIDV1_dataset_G99.csv", stringsAsFactors = FALSE, sep="\t", row.names = 1, header=FALSE)

#colnames(XIDV1_dataset)<-paste(XIDV1_dataset[1,], 1:dim(XIDV1_dataset)[2], sep="")
#XIDV1_dataset<-XIDV1_dataset[,-grep("0610005C13Rik", colnames(XIDV1_dataset))]

#XIDV1_names<-read.csv("./XIDV1sample_names_G99.csv", header=FALSE, stringsAsFactors = FALSE)

#XIDV1_names<-XIDV1_names[,1]
#XIDV1_names<-gsub(".counts.csv", "", XIDV1_names)
#colnames(XIDV1_dataset)<-XIDV1_names

#write.csv(XIDV1_dataset, "./XIDV1_datasetNew_G99.csv")
```
...
findEqPositions<-function(df1, df2){ #find which positions from df2 correspond to the ones in df1 df1 and df2 are data frames of gene expression where each row is a gene and each column is a position. Empty positions are columns of NAs. This returns a list of 3 elements: the range of positions of df2 which is equivalent to positions in df1, the vector of correlation coefficients for all genes in df1 and df2 taking the range of slices from df2 that give the maximum sum of correlations when compared to slices in df1, and the data frame of correlation coefficients for all genes in df1 and df2 taking all possible ranges of slices
  ...
  for(i in 1:(1+dim(df2)[2]-dim(df1)[2])){
    ...
    #if(median(cors, na.rm=T)>maxCorsSum){
      maxCorsSum<-sum(cors, na.rm=T)
      #maxCorsSum<-median(cors, na.rm=T)
      corsMax=cors
...
```{r}
#cellPropsNusvr<-read.csv("./supp_tables/cellTypesProps_nusvr_DV.csv", row.names=1)
cellPropsNusvr<-read.csv("./supp_tables/cellTypesProps_nusvr_T2_DV.csv", row.names=1)
...
#cellPropsNnls<-read.csv("./supp_tables/cellTypesProps_nnls_DV.csv", row.names=1)
cellPropsNnls<-read.csv("./supp_tables/cellTypesProps_nnls_T2_DV.csv", row.names=1)
...
#pdf(file = "./figures_pdf2/FigCellDeconvNNLS_DV_rev.pdf", width=5, height = 5)
pdf(file = "./figures_pdf2/FigCellDeconvNNLS_T2_DV_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNnls)){
  plot(1:dim(cellPropsNnls)[1], cellPropsNnls[,cell], xlim=c(0, dim(cellPropsNnls)[1]), main=paste("Nnls", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, 0.5))
  lines(locfit(cellPropsNnls[,cell]~lp(1:dim(cellPropsNnls)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNnls)), col="red")
}

par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNnls)){
  plot(1:dim(cellPropsNnls)[1], cellPropsNnls[,cell], xlim=c(0, dim(cellPropsNnls)[1]), main=paste("Nnls", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, max(cellPropsNnls[,cell])))
  lines(locfit(cellPropsNnls[,cell]~lp(1:dim(cellPropsNnls)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNnls)), col="red")
}
dev.off()

#pdf(file = "./figures_pdf2/FigCellDeconvNuSVR_DV_rev.pdf", width=5, height = 5)
pdf(file = "./figures_pdf2/FigCellDeconvNuSVR_T2_DV_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNusvr)){
  plot(1:dim(cellPropsNusvr)[1], cellPropsNusvr[,cell], xlim=c(0, dim(cellPropsNusvr)[1]), main=paste("Nusvr", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, 0.5))
  lines(locfit(cellPropsNusvr[,cell]~lp(1:dim(cellPropsNusvr)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNusvr)), col="red")
}

par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNusvr)){
  plot(1:dim(cellPropsNusvr)[1], cellPropsNusvr[,cell], xlim=c(0, dim(cellPropsNusvr)[1]), main=paste("Nusvr", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, max(cellPropsNusvr[,cell])))
  lines(locfit(cellPropsNusvr[,cell]~lp(1:dim(cellPropsNusvr)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNusvr)), col="red")
}
dev.off()
...
#write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_NusvrDV.csv")
write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_Nusvr_T2_DV.csv")

binMat_CT<-binarizeData(data=t(cellPropsNnls))
CT_table<-autocorrelationTest(data=binMat_CT, maxLag=(dim(binMat_CT)[2]-10), filter=rep(1, dim(binMat_CT)[1]))
#write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_NnlsDV.csv")
write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_Nnls_T2_DV.csv")

pdf(file = "./figures_pdf2/FigCellDeconvNuSVR_HM_T2_DV_rev.pdf", width=5, height = 5)
...
totalReads<-read.csv("./DV_totalReads_G99.csv", header=FALSE, stringsAsFactors = FALSE)
...
totalReads<-c(totalReads, totalReadsXIDV1)
...
expr<-as.matrix(DV_dataset[1:(dim(DV_dataset)[1]-5),])
...
col[63:117]<-"blue" #DV2
col[118:201]<-"green" #DV3
col[202:267]<-"purple" #XIDV1 or DV4
...
plotQCstats(statsData = stats, rows = c(8:12), x=as.numeric(log10(stats[7,]+1)), xlab="log10(total reads+1)", col=col, legend_names=c("DV1", "DV2", "DV3", "DV4"), legend_colors=c("red", "blue", "green", "purple"), ycut = ythresholds, xcut = xthresholds)
```


**Figure 1.** Quality statistics of samples

```{r Quality_checkMod-batch3, echo=FALSE, message=FALSE, warning=FALSE}
...
DV_dataset<-cbind(DV_dataset, XIDV1_dataset)
#write.csv(DV_dataset, "./DV_datasetNew.2_G99.csv")
...
plotQCstats(statsData = stats, rows = c(8:12), x=as.numeric(log10(stats[7,]+1)), xlab="log10(total reads+1)", col=col, legend_names=c("DV1", "DV2", "DV3"), legend_colors=c("red", "blue", "green"), ycut = ythresholds, xcut = xthresholds)
```
...
pdf(file = "./figures_pdf2/FigS1E_DV.pdf", width=5, height = 5)
...
#write.csv(marker_heat, "./supp_tables/log10NeuronalMksDV.csv")
```
...
par(mfrow=c(3, 2), mar=c(4,2,2,2))
plotQCstats(statsData = stats, rows = c(8:12), x=as.numeric(log10(stats[7,]+1)), xlab="log10(total reads+1)", col=col, legend_names=c("Not Selected", "Selected_DV1", "Selected_DV2", "Selected_DV3"), legend_colors=c("black", "red", "blue", "green"), ycut = ythresholds, xcut = xthresholds)
...
pdf(file = "./figures_pdf2/FigS1A_DV.pdf", width=5, height = 5)
boxplot(as.numeric(stats[8, selection]), main=rownames(stats)[8], ylim=c(40, 100), xlab="DV axis")
dev.off()

pdf(file = "./figures_pdf2/FigS1C_DV.pdf", width=5, height = 5)
boxplot(as.numeric(log10(stats[11, selection]+1)), main=paste("log10", rownames(stats)[11], sep=""), ylim=c(2,5), xlab="DV axis")
dev.off()

pdf(file = "./figures_pdf2/FigS1B_DV.pdf", width=5, height = 5)
boxplot(as.numeric(stats[12, selection]), main=rownames(stats)[12], ylim=c(0, 20), xlab="DV axis")
dev.off()

pdf(file = "./figures_pdf2/FigS1D_DV.pdf", width=5, height = 5)
boxplot(as.numeric(log10(stats[7, selection]+1)), main="Total no. of reads (log10)", ylim=c(4.5, 8), xlab="DV axis")
dev.off()
...
#write.csv(stats, "./supp_tables/newDV_QCstats_G99.csv")
#write.csv(stats[,selection], "./supp_tables/newDV_selected_QCstats_G99.csv")

#batches 2 and 3 are reversed

position<-c(rev(1:62), rev(1:55), 1:66)
sel_positions<-position[selection]
selected<-expr[, selection] #select samples with nice quality stats

batch<-col[which(col!="black")]
batch[which(batch=="red")]<-"DV1"
batch[which(batch=="blue")]<-"DV2"
batch[which(batch=="green")]<-"DV3"
rawDS<-selected
colnames(rawDS)<-paste(batch, sel_positions, sep="-")
#write.csv(rawDS, "./supp_tables/newDV_rawQCdone_G99.csv")
rm(rawDS)

par(mfrow=c(1, 1))
plot(c(length(sel_positions[sel_positions<35]), length(sel_positions[sel_positions>=35])), xaxt='n', xlab="", ylab="", ylim=c(0, 150), pch=19, cex=4, col=c("red", "blue"), main="Number of samples per area")
...
#write.csv(normalized, "./newDVnormalized.rpmJust-0s_G99.csv")

batch<-col[which(col!="black")]
batch[which(batch=="red")]<-"DV1"
batch[which(batch=="blue")]<-"DV2"
batch[which(batch=="green")]<-"DV3"
#write.csv(normalized[, which(batch=="DV1")], "./supp_tables/newDVnormalized_b1.rpmJust-0s_G99.csv")
#write.csv(normalized[, which(batch=="DV2")], "./supp_tables/newDVnormalized_b2.rpmJust-0s_G99.csv")
#write.csv(normalized[, which(batch=="DV3")], "./supp_tables/newDVnormalized_b3.rpmJust-0s_G99.csv")
...
initSamplesPerBatch<-c(62, 55, 66) #initial number of samples we had per batch
names(initSamplesPerBatch)<-c("DV1", "DV2", "DV3")

#write.csv(getSamplesPerBatch(batchVector = batch, totalSamplesPerBatch = initSamplesPerBatch), "./supp_tables/DVgoodSamplesPerBatch.csv")
```
...
oneRepGenes<-data.frame(b1=rowSums(normalized[,which(batch=="DV1")])>1, b2=rowSums(normalized[,which(batch=="DV2")])>1, b3=rowSums(normalized[,which(batch=="DV3")])>1)
...
HVGsB1<-rownames(getHVGs(data=normalized[,which(batch=="DV1")]))[1:3000]
HVGsB2<-rownames(getHVGs(data=normalized[,which(batch=="DV2")]))[1:3000]
HVGsB3<-rownames(getHVGs(data=normalized[,which(batch=="DV3")]))[1:3000]

#change to top 3000 per replicate
HVGs<-intersect(HVGsB1, HVGsB2)
HVGs<-intersect(HVGs, HVGsB3)
HVGs<-normalized[which(rownames(normalized) %in% HVGs),]

#remove mitochondrial and ribosomal genes
#HVGs<-HVGs[-grep("mt-", rownames(HVGs)),]
#HVGs<-HVGs[-grep("Rps", rownames(HVGs)),]
...
for(i in 8:12){
  plotXYcorrelation(x=sel_positions, y=as.numeric(stats[i,selection]), main=c(paste("DV position vs.", rownames(stats)[i])), ylab=rownames(stats)[i], xlab="DV position", col=color_selection[-which(color_selection=="black")])
}

plotXYcorrelation(x=sel_positions, y=as.numeric(log10(stats[7, selection]+1)), main="DV position vs. Total no. of reads (log10)", ylab="Total no. of reads (log10)", xlab="DV position", col=color_selection[-which(color_selection=="black")])
...
legend("bottomleft", c("Dorsal", "Ventral"), pch=19, col=alpha(c("lightblue", "darkblue"), 0.3), cex=0.8)

#See if PC 1 and 2 correlate with position

par(mfrow=c(1, 2), mar=c(4, 4, 4, 2))

plotXYcorrelation(x=samples_pcomp2$x[,1], y=sel_positions, main="PC1 vs DV position", ylab="DV position", xlab="PC1", col=color_selection[-which(color_selection=="black")])

plotXYcorrelation(x=samples_pcomp2$x[,2], y=sel_positions, main="PC2 vs DV position", ylab="DV position", xlab="PC2", col=color_selection[-which(color_selection=="black")])
```
...
heatmap.2(as.matrix(marker_heat), xlab="Position DV", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, key.xlab = "log10geneExp", Colv=F)
...
par(mfrow=c(2, 2))
plotGenesInEachBatch(data=log10(normalized+1), genes=c("Cnga2", "Epas1", "Ric8b", "Rtp1", "Rtp2", "Cnga4", "Gap43", "Omp", "Gng13", "Gnal", "Gnb1", "Gnai2", "Gnao1", "Six1", "Alcam", "Ano2", "Adcy3", "Chl1", "Trp63", "Slc2a3", "Neurog1", "Plcb2", "Ascl1", "Trpc2", "Reep1", "Krt5", "Sema3a", "Nrp1", "Olfr1507", "Olfr449"), x=sel_positions, batches=c("DV1", "DV2", "DV3"), batchColors=c("red", "blue", "green"), xlab="position DV", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))

par(mfrow=c(2, 2))
plotGenesInEachBatch(data=log10(normalized+1), genes=c("Acsm4", "Ncam2", "Nqo1"), x=sel_positions, batches=c("DV1", "DV2", "DV3"), batchColors=c("red", "blue", "green"), xlab="position DV", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))

par(mfrow=c(2, 2))
plotGenesInEachBatch(data=normalized, genes=c("Acsm4"), x=sel_positions, batches=c("DV1", "DV2", "DV3"), batchColors=c("red", "blue", "green"), xlab="position DV", ylab="rpm expression", ylim=c(0, max(normalized[c("Acsm4"),])))
```
...
heatmap.2(HVGs, xlab="Position DV", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, key.xlab = "log10geneExp", Colv = F)
```
...
#heatmap.2(cor_HVGs, Colv = NA, Rowv=NA, trace='none', key.xlab="Cor.coefficient", key.ylab=NA, col=viridis_pal(), breaks=seq(0,1,0.1), cexRow = 0.5, cexCol = 0.5)
...
#HVGs<-log10(getHVGs(normalized[,grep("DV2", colnames(normalized))])[1:1000,]+1)
normHVGs<-normalized[rownames(HVGs),]

tmp3<-batchSubset(data=normHVGs, batch="DV3", positions=sel_positions)
tmp2<-batchSubset(data=normHVGs, batch="DV2", positions=sel_positions)
tmp1<-batchSubset(data=normHVGs, batch="DV1", positions=sel_positions)

#tmp2=shortest dataset
findEqPositions(tmp2, tmp3)[[1]]
boxplot(findEqPositions(tmp2, tmp3)[[3]][,-1], main = "coefficients distribution b3")
#abline(h=mean(findEqPositions(tmp3, tmp2)[[2]]), col="red")
findEqPositions(tmp2, tmp1)[[1]]
boxplot(findEqPositions(tmp2, tmp1)[[3]][,-1], main = "coefficients distribution b1")
...
normalized<-cbind(normalized[,colnames(normalized) %in% paste("DV1", findEqPositions(tmp2, tmp1)[[1]][1]:findEqPositions(tmp2, tmp1)[[1]][2], sep="-")], normalized[,colnames(normalized) %in% paste("DV2", 1:54, sep="-")], normalized[,colnames(normalized) %in% paste("DV3", findEqPositions(tmp2, tmp3)[[1]][1]:findEqPositions(tmp2, tmp3)[[1]][2], sep="-")])

#order normalized by position

normalized["sel_positions",]<-c(as.numeric(gsub("DV1-", "", colnames(normalized)[grep("DV1", colnames(normalized))]))-(findEqPositions(tmp2, tmp1)[[1]][1]-1), as.numeric(gsub("DV2-", "", colnames(normalized)[grep("DV2", colnames(normalized))])), as.numeric(gsub("DV3-", "", colnames(normalized)[grep("DV3", colnames(normalized))]))-(findEqPositions(tmp2, tmp3)[[1]][1]-1))
...
batch<-rep("DV1", dim(normalized)[2])
batch[grep("DV2", colnames(normalized))]<-"DV2"
batch[grep("DV3", colnames(normalized))]<-"DV3"

batch_col<-rep("red", length(batch))
batch_col[which(batch=="DV2")]<-"blue"
batch_col[which(batch=="DV3")]<-"green"

AvPerGene<-t(apply(normalized[-dim(normalized)[1],], 1, function(i){tapply(i, batch, mean)}))

normalized[-dim(normalized)[1],which(batch=="DV1")]<-apply(normalized[-dim(normalized)[1],which(batch=="DV1")], 2, function(x){x*rowMeans(AvPerGene)/AvPerGene[,"DV1"]})
normalized[-dim(normalized)[1],which(batch=="DV2")]<-apply(normalized[-dim(normalized)[1],which(batch=="DV2")], 2, function(x){x*rowMeans(AvPerGene)/AvPerGene[,"DV2"]})
normalized[-dim(normalized)[1],which(batch=="DV3")]<-apply(normalized[-dim(normalized)[1],which(batch=="DV3")], 2, function(x){x*rowMeans(AvPerGene)/AvPerGene[,"DV3"]})
...
col=colorRampPalette(c("lightblue", "darkblue"))
plot(samples_pcomp2$x[,1], samples_pcomp2$x[,2], col=alpha(col(96)[normalized["sel_positions",]], 0.3), xlab="PC1", ylab="PC2", pch=19, cex=1)
legend("bottomleft", c("Dorsal", "Ventral"), pch=19, col=alpha(c("lightblue", "darkblue"), 0.3), cex=0.8)
...
plotXYcorrelation(x=samples_pcomp2$x[,1], y=normalized["sel_positions",], main="PC1 vs DV position", ylab="DV position", xlab="PC1", col=batch_col)

plotXYcorrelation(x=samples_pcomp2$x[,2], y=normalized["sel_positions",], main="PC2 vs DV position", ylab="DV position", xlab="PC2", col=batch_col)
```
...
plotGenesInEachBatch(data=log10(normalized+1), genes=c("Cnga2", "Epas1", "Ric8b", "Rtp1", "Rtp2", "Cnga4", "Gap43", "Omp", "Gng13", "Gnal", "Gnb1", "Gnai2", "Gnao1", "Six1", "Alcam", "Ano2", "Adcy3", "Chl1", "Trp63", "Slc2a3", "Neurog1", "Plcb2", "Ascl1", "Trpc2", "Reep1", "Krt5", "Sema3a", "Nrp1", "Olfr1507", "Olfr449"), x=normalized["sel_positions",], batches=c("DV1", "DV2", "DV3"), batchColors=c("red", "blue", "green"), xlab="position DV", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))

par(mfrow=c(2, 2))

plotGenesInEachBatch(data=log10(normalized+1), genes=c("Acsm4", "Ncam2", "Nqo1"), x=normalized["sel_positions",], batches=c("DV1", "DV2", "DV3"), batchColors=c("red", "blue", "green"), xlab="position DV", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))

plotGenesInEachBatch(data=normalized, genes=c("Acsm4"), x=normalized["sel_positions",], batches=c("DV1", "DV2", "DV3"), batchColors=c("red", "blue", "green"), xlab="position DV", ylab="rpm expression", ylim=c(0, max(normalized["Acsm4",])))


#PC1 biggest loadings
par(mfrow=c(2, 2))

plotGenesInEachBatch(data=log10(normalized+1), genes=rownames(samples_pcomp2$rotation[order(-abs(samples_pcomp2$rotation[,1])),])[1:10], x=normalized["sel_positions",], batches=c("DV1", "DV2", "DV3"), batchColors=c("red", "blue", "green"), xlab="position DV", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))
```
...
#position<-as.numeric(gsub("DV..", "", position))
...
#write.csv(rbind(HVGs, sel_positions=normalized["sel_positions",]), "./supp_tables/DVlog10HVGs.csv")
...
#pdf(file = "./figures_pdf2/Fig1B_DV.pdf", width=5, height = 5)
heatmap.2(cor_HVGs, Colv = NA, Rowv=NA, trace='none', key.title=NA, key.ylab=NA, key.xlab="Cor.coefficient", breaks=seq(0,1,0.1), col=viridis_pal(), cexRow = 0.5, cexCol = 0.5)
#dev.off()
...
pdf(file = "./figures_pdf2/Fig1B_alt_DV.pdf", width=5, height = 5)
boxplot(unlist(cor_list), ylab="rho", xlab="DV axis", ylim=c(0, 1))
boxplot(cor_list, ylab="rho", xlab="DV position", ylim=c(0, 1), cex.lab=0.6, cex.axis=0.6)
...
plotMeanDetectedGenesPerPosition(data=normalized[-which(rownames(normalized)=="sel_positions"),], thresh=10, positions=normalized["sel_positions",], errorBars="SDV", xlab="position DV")

normORs<-rbind(normalized[grep("Olfr", rownames(normalized)),], sel_positions=normalized["sel_positions",])
plotMeanDetectedGenesPerPosition(data=normORs[-which(rownames(normORs)=="sel_positions"),], thresh=10, positions=normORs["sel_positions",], errorBars="SDV", xlab="position DV")
rm(normORs)
```
...
#write.csv(normalized, "./newDVnormalized_rpm_repsAvg_G99.csv")

#Use only first 60 slices (dont think so)

#normalized<-normalized[,1:60]
...
pdf(file = "./figures_pdf2/Fig1E_DV_agOlfrs_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(marker in agGenes$genes[grep("Olfr", agGenes$genes)][agGenes$genes[grep("Olfr", agGenes$genes)] %in% rownames(normalized)]){
  ...
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(normalized[marker,])+1))
...
for(marker in c("Cnga2", "Epas1", "Ric8b", "Rtp1", "Rtp2", "Cnga4", "Gap43", "Omp", "Gng13", "Gnal", "Gnb1", "Gnai2", "Gnao1", "Six1", "Alcam", "Ano2", "Adcy3", "Chl1", "Trp63", "Slc2a3", "Neurog1", "Plcb2", "Ascl1", "Trpc2", "Reep1", "Krt5", "Sema3a", "Nrp1", "Olfr1507", "Olfr449")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(normalized)[2], log10(normalized[marker,]+1), xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position DV", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normalized+1))))
}
...
pdf(file = "./figures_pdf2/Fig1E_DV.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
...
pdf(file = "./figures_pdf2/Fig1E_DV_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
...
pdf(file = "./figures_pdf2/NeuroMksHM_DV_rev.pdf", width=7, height = 7)
heatmap.2(as.matrix(heatNeuroMks), Colv = NA, xlab="Position DV", trace='none', key.xlab="log10meanRPMnorm", key.ylab=NA, key.title=NA, col=viridis(100), cexRow = 0.8, cexCol = 0.8)
dev.off()

pdf(file = "./figures_pdf2/Fig1E_DV_QCcheck_T3_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
...
write.csv(normalized[c("Acsm4", "Ncam2", "Nqo1", "Reg3g"),], "./supp_tables/spatialMarkersDV.csv")

svg(file = "./figures_pdf2/Fig1E_DV.svg", width=5, height = 5)
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(normalized[marker,])+1))
...
cairo_ps("./figures_pdf2/Fig1E_DV.eps", width = 5, height = 5)
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(normalized[marker,])+1))
...
```{r}
mOSNpropsNusvr<-read.csv("./supp_tables/DV_mOSNproportions_NuSVR.csv")
mOSNpropsNnls<-read.csv("./supp_tables/DV_mOSNproportions_nnls.csv")

pdf(file = "./figures_pdf2/Fig3A_DV.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in c("mOSNs", "others")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(mOSNpropsNusvr$X+1, mOSNpropsNusvr[,cell], xlim=c(0, length(mOSNpropsNusvr$X)+1), main=paste("NuSVR", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, 1))
  lines(locfit(mOSNpropsNusvr[,cell]~lp(mOSNpropsNusvr$X+1, deg = 2, nn=1), data=as.data.frame(mOSNpropsNusvr)), col="red")
  
  plot(mOSNpropsNnls$X+1, mOSNpropsNnls[,cell], xlim=c(0, length(mOSNpropsNnls$X)+1), main=paste("nnls", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, 1))
...
```{r}
mOSNpropsNusvr<-read.csv("./supp_tables/DV_NeuronsProportions_NuSVR.csv")
mOSNpropsNnls<-read.csv("./supp_tables/DV_NeuronsProportions_nnls.csv")

pdf(file = "./figures_pdf2/Fig3A.2_DV.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in c("neurons", "otherCells")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(mOSNpropsNusvr$X+1, mOSNpropsNusvr[,cell], xlim=c(0, length(mOSNpropsNusvr$X)+1), main=paste("NuSVR", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, 1), col=alpha(1, 0.3))
  lines(locfit(mOSNpropsNusvr[,cell]~lp(mOSNpropsNusvr$X+1, deg = 2, nn=1), data=as.data.frame(mOSNpropsNusvr)), col="red")
  
  plot(mOSNpropsNnls$X+1, mOSNpropsNnls[,cell], xlim=c(0, length(mOSNpropsNnls$X)+1), main=paste("nnls", cell), xlab="position DV", ylab="proportion", pch=19, ylim=c(0, 1), col=alpha(1, 0.3))
  lines(locfit(mOSNpropsNnls[,cell]~lp(mOSNpropsNnls$X+1, deg = 2, nn=1), data=as.data.frame(mOSNpropsNnls)), col="red")
}
dev.off()
...
#genes from https://www.biorxiv.org/content/biorxiv/early/2020/03/28/2020.03.25.009084.full.p#df
...
pdf(file = "./figures_pdf2/Fig3B_DV.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(marker in geneList$gene){
  #plot(1:dim(normalized)[2], log10(normalized[marker,]+1), xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position DV", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normalized[geneList$gene,]+1))))
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position DV", ylab="log10 mean rpm expression", pch=19)
  lines(locfit(normalized[marker,]~lp(1:dim(normalized)[2], deg = 2, nn=1), data=as.data.frame(normalized)), col="red")
}
dev.off()

write.csv(normalized[c("Fgf20", "Wfdc18", "Rps21"),], "./supp_tables/someGenesFromOtherCells_DV.csv")

cairo_ps("./figures_pdf2/Fig3B_DV.eps", width = 5, height = 5)
par(mfrow=c(2, 2))
for(marker in geneList$gene){
  #plot(1:dim(normalized)[2], log10(normalized[marker,]+1), xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position DV", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normalized[geneList$gene,]+1))))
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position DV", ylab="log10 mean rpm expression", pch=19)
...
pdf(file = "./figures_pdf2/FigS1E_DV_alt.pdf", width=5, height = 5)
heatmap.2(as.matrix(marker_heat), xlab="", trace='none', key.title=NA, key.ylab=NA, col=viridis(length(seq(0,6,0.1))-1), cexRow = 0.8, key.xlab = "log10geneExp", labCol = FALSE, breaks=seq(0,6,0.1), Rowv=F)
dev.off()
...
pdf(file = "./figures_pdf2/Fig1D_DV.pdf", width=5, height = 5)
heatmap.2(as.matrix(marker_heat), xlab="Position DV", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, cexCol=0.5, key.xlab = "log10meanRPM", Colv = F, Rowv=F)
dev.off()

write.csv(marker_heat, "./supp_tables/log10MOEMarkersDV.csv")
```
...
write.csv(normalized, "./supp_tables/newDV_ORS_normalized_rpm_repsAvg_mks_G99.csv")
```


```{r OSN_markers5, echo=FALSE, message=FALSE, warning=FALSE}
#didn't detect Olfr247
par(mfrow=c(2, 2))
...
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  plot(1:dim(normCanonMks)[2], log10(normCanonMks[marker,]+1), xlim=c(0, dim(normCanonMks)[2]+1), main=marker, xlab="position DV", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normCanonMks+1))))
}

par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  plot(1:dim(normCanonMks)[2], normCanonMks[marker,], xlim=c(0, dim(normCanonMks)[2]+1), main=marker, xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(normCanonMks[marker,])+1))
}
```
...
heatmap.2(as.matrix(marker_heat), xlab="Position DV", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, key.xlab = "log10meanRPM", Colv = F)
...
col=colorRampPalette(c("lightblue", "darkblue"))
plot(samples_pcompAvgs$x[,1], samples_pcompAvgs$x[,2], col=alpha(col(55)[1:dim(norm_complete)[2]], 0.3), xlab="PC1", ylab="PC2", pch=19, cex=1)
legend("bottomleft", c("Dorsal", "Ventral"), pch=19, col=alpha(c("lightblue", "darkblue"), 0.3), cex=0.8)
...
plotXYcorrelation(x=samples_pcompAvgs$x[,1], y=1:dim(norm_complete)[2], main="PC1 vs DV position", ylab="DV position", xlab="PC1", col=1)

plotXYcorrelation(x=samples_pcompAvgs$x[,2], y=1:dim(norm_complete)[2], main="PC2 vs DV position", ylab="DV position", xlab="PC2", col=1)
```
...
col=colorRampPalette(c("lightblue", "darkblue"))
plot(samples_pcompAvgs$x[,1], samples_pcompAvgs$x[,2], col=alpha(col(55)[1:dim(normalized2)[2]], 0.3), xlab="PC1", ylab="PC2", pch=19, cex=1)
legend("bottomleft", c("Dorsal", "Ventral"), pch=19, col=alpha(c("lightblue", "darkblue"), 0.3), cex=0.8)
...
plotXYcorrelation(x=samples_pcompAvgs$x[,1], y=1:dim(normalized2)[2], main="PC1 vs DV position", ylab="DV position", xlab="PC1", col=1)

plotXYcorrelation(x=samples_pcompAvgs$x[,2], y=1:dim(normalized2)[2], main="PC2 vs DV position", ylab="DV position", xlab="PC2", col=1)
...
DVdists<-as.matrix(dist(1:dim(HVGsAvgs)[2]))
DVdists.inv<-1/DVdists
diag(DVdists.inv)<-0

moransIs<-sapply(1:dim(HVGsAvgs)[1], function(x){Moran.I(HVGsAvgs[x,], DVdists.inv)$observed})

moransIsExp<-sapply(1:dim(HVGsAvgs)[1], function(x){Moran.I(HVGsAvgs[x,], DVdists.inv)$expected})

moransIsPvals<-sapply(1:dim(HVGsAvgs)[1], function(x){Moran.I(HVGsAvgs[x,], DVdists.inv)$p.value})
...
tmp<-data.frame(DVmoransIs=moransIs, DVmoransIsPvals=moransIsPvals)
rownames(tmp)<-rownames(HVGsAvgs)
write.csv(tmp, "./supp_tables/DVmoransI.csv")

```

##Differential expression along DV axis

In order to find genes which are differentially expressed across the DV axis, I performed an autocorrelation test. So if a gene "g"'s expression at positions "x" correlated significantly (p<0.05) with gene expression at positions ("x+1"); and the correlation coefficient was bigger than 0.4, g is differentially expressed.
...
#heatmap.2(as.matrix(heatDEGs2.1), Colv = NA, xlab="Position DV", trace='none', key.xlab="normalized gene expression", key.ylab=NA, key.title=NA, col=viridis(100), cexRow = 0.8, cexCol = 0.8, breaks=seq(0,1,0.1))

heatmap.2(as.matrix(heatDEGs2.1), Colv = NA, xlab="Position DV", trace='none', key.xlab="log10meanRPMnorm", key.ylab=NA, key.title=NA, col=viridis(100), cexRow = 0.8, cexCol = 0.8)
```
...
write.csv(DEGsfile, "./supp_tables/newDVDEORs.autocor.binMatFDR0.01filt_NormMks_G99.csv")
...
#write.csv(dframe, "./clusters_deORsDV.csv")
```
...
for(i in (1:length(c1NoDE))){
  plot(position, log10(normalized[which(rownames(normalized)==c1NoDE[i]),]+1), main=c1NoDE[i], xlab="position DV", ylab="log10 mean rpm expression", pch=19)
...
dpt <- DPT(ORsDiffmap, tips=4) #pseudotime
...
**Figure PCA**
**Figure 19.** PCA plot for class 1 and 2 ORs. Class 1 in red, class 2 in blue
...
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORs, ylim=c(0, 1), ylab="fitted norm. log10 mean rpm expression", xlab="position DV")
...
#chisq.test(tbl2)
DEGsfile<-cbind(DEGsfile, clusters_deORs)
write.csv(DEGsfile, "./supp_tables/newDVDEORs.autocor.binMatFDR0.01filt_NormMks_G99.csv")
```
...
write.csv(normalized, "./supp_tables/newDV_AllGenes_normalized_rpm_repsAvg_mks.csv")
...
plot(ts, main="Acsm4")
#par(mfrow=c(1, 3))
pdf(file = "./figures_pdf2/Fig2A.1.pdf", width=5, height = 5)
plot(position, log10(normalized["Acsm4",]+1), xlim=c(0, max(position)+1), main="Acsm4", xlab="position DV", ylab="log10 mean rpm expression", pch=19)
abline(h=median(log10(normalized["Acsm4",]+1)), col="red")
dev.off()

cairo_ps("./figures_pdf2/Fig2A.1.eps", width = 5, height = 5)
plot(position, log10(normalized["Acsm4",]+1), xlim=c(0, max(position)+1), main="Acsm4", xlab="position DV", ylab="log10 mean rpm expression", pch=19)
abline(h=median(log10(normalized["Acsm4",]+1)), col="red")
dev.off()

pdf(file = "./figures_pdf2/Fig2A.2.pdf", width=5, height = 5)
plot(position, binMat["Acsm4",], xlim=c(0, max(position)+1), main="Acsm4", xlab="position DV", ylab="binarized expression", pch=19)
dev.off()

cairo_ps("./figures_pdf2/Fig2A.2.eps", width = 5, height = 5)
plot(position, binMat["Acsm4",], xlim=c(0, max(position)+1), main="Acsm4", xlab="position DV", ylab="binarized expression", pch=19)
dev.off()

pdf(file = "./figures_pdf2/Fig2A.3.pdf", width=5, height = 5)
acf(ts, lag.max=(dim(binMat)[2]-10))
dev.off()
...
```{r extraDEGs, message=FALSE, warning=FALSE}
extraDEGs<-read.csv("./3D/DVextraDEGs.csv", stringsAsFactors = F, row.names=1)
top20extraDEGs<-genes.table[which(genes.table$genes.names %in% extraDEGs$x),]
top20extraDEGs<-top20extraDEGs[1:20,]

write.csv(top20extraDEGs, "./supp_tables/top20DVextraDEGs.csv")
```
...
write.csv(DEGsfile, "./supp_tables/newDVDEGs.autocor.binMatFDR0.01filt_G99.csv")
...
write.csv(truePatts_table, "./supp_tables/SDEtest_DEGsTruePatts_DV.csv")
...
pdf("./figures_pdf2/mks_CTtrend_DV_rev.pdf")

par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Nqo1")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(DEGs2test)[2], DEGs2test[marker,], xlim=c(0, dim(DEGs2test)[2]+1), main=marker, xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(DEGs2test[marker,])+1))
  lines(locfit(DEGs2test[marker,]~lp(1:dim(DEGs2test)[2], deg = 2, nn=1), data=as.data.frame(DEGs2test)), col="red")
}

par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Nqo1")){
  plot(1:dim(geneExpPat_due2CTdist)[2], geneExpPat_due2CTdist[marker,], xlim=c(0, dim(geneExpPat_due2CTdist)[2]+1), main=paste(marker, "CT pattern"), xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(geneExpPat_due2CTdist[marker,])+1))
...
#par(mfrow=c(2, 2))
#for(marker in c("Acsm4", "Nqo1")){
#  plot(1:dim(trueExpPatterns)[2], trueExpPatterns[marker,], xlim=c(0, dim(trueExpPatterns)[2]+1), main=marker, xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(min(trueExpPatterns[marker,]), max(trueExpPatterns[marker,])+1))
...
#CTpredictableDEGs<-names(spearmanFDRs)[which(spearmanFDRs<0.01 & spearmanCors>=0.45)]

is.predictable<-names(spearmanFDRs) %in% CTpredictableDEGs
write.csv(data.frame(geneName=names(spearmanFDRs), spearmanRho=spearmanCors, pval=spearmanPvals, fdr=spearmanFDRs, is.predictable=is.predictable), "./supp_tables/DEGsCTpredictability_DV_rev.csv")

pdf("./figures_pdf2/predictableOlfrs_DV_rev.pdf")
par(mfrow=c(2, 2))
for(marker in CTpredictableDEGs[grep("Olfr", CTpredictableDEGs)]){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(DEGs2test)[2], DEGs2test[marker,], xlim=c(0, dim(DEGs2test)[2]+1), main=paste(marker, "original pattern"), xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(DEGs2test[marker,])+1))
  lines(locfit(DEGs2test[marker,]~lp(1:dim(DEGs2test)[2], deg = 2, nn=1), data=as.data.frame(DEGs2test)), col="red")
}

par(mfrow=c(2, 2))
for(marker in CTpredictableDEGs[grep("Olfr", CTpredictableDEGs)]){
  plot(1:dim(geneExpPat_due2CTdist)[2], geneExpPat_due2CTdist[marker,], xlim=c(0, dim(geneExpPat_due2CTdist)[2]+1), main=paste(marker, "pattern due to cell types dist."), xlab="position DV", ylab="mean rpm expression", pch=19, ylim=c(0, max(geneExpPat_due2CTdist[marker,])+1))
  lines(locfit(geneExpPat_due2CTdist[marker,]~lp(1:dim(geneExpPat_due2CTdist)[2], deg = 2, nn=1), data=as.data.frame(geneExpPat_due2CTdist)), col="red")
}
...
write.csv(geneExpPat_due2CTdist, "./supp_tables/DEGsExpPat_due2CTdist_DV.csv")
...
spearmanCoeffs<-matrix(NA, dim(DEGsbox)[1], dim(cellPropsNusvr)[2])
spearmanPvals<-matrix(NA, dim(DEGsbox)[1], dim(cellPropsNusvr)[2])
rownames(spearmanCoeffs)<-rownames(DEGsbox)
rownames(spearmanPvals)<-rownames(DEGsbox)
colnames(spearmanCoeffs)<-colnames(cellPropsNusvr)
colnames(spearmanPvals)<-colnames(cellPropsNusvr)

for(gene in rownames(DEGsbox)){
  tmp<-sapply(colnames(cellPropsNusvr), function(x){
    cor.test(cellPropsNusvr[,x], DEGsbox[gene,], method="spearman")
  })
...
par(mfrow=c(3, 3))
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORsMinCS100, ylim=c(0, 1), ylab="fitted norm. log10 mean expression", xlab="position DV")
...
par(mfrow=c(1, 1))
plot(x=cluster2index$myIndx, y=cluster2index$zone.index, col=alpha(cluster2index$Cluster, 0.3), xlab="our DV Indexes", ylab="Zone index by Miyamichi", pch=19)
...
par(mfrow=c(3, 3))
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORsMinCS25, ylim=c(0, 1), ylab="fitted norm. log10 mean expression", xlab="position DV")
...
classCol2<-classCol
classCol2[-which(names(classCol2) %in% ZoneIndexes$`modern gene name`)]<-NA
...
write.csv(nGenesPerClust, "./supp_tables/nGenesPerClust_DV.csv")

ORsUmap<-umap(deORs) #normalized Differentially Expressed OR genes

pdf(file = "./figures_pdf2/Fig2B.1.pdf", width=5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(clusters_deORs, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

cairo_ps("./figures_pdf2/Fig2B.1.eps", width = 5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(clusters_deORs, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

pdf(file = "./figures_pdf2/Fig4B.1.pdf", width=5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(ORcols, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

cairo_ps("./figures_pdf2/Fig4B.1.eps", width = 5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(ORcols, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(zoneCol, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")

pdf(file = "./figures_pdf2/Fig4B.2.pdf", width=5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(classCol, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
legend("topleft", c("Class 1", "Class 2"), pch=19, col=alpha(c("red", "blue"), 0.3))
dev.off()

cairo_ps("./figures_pdf2/Fig4B.2.eps", width = 5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(classCol, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
legend("topleft", c("Class 1", "Class 2"), pch=19, col=alpha(c("red", "blue"), 0.3))
dev.off()

plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(classCol2, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
legend("topleft", c("Class 1", "Class 2"), pch=19, col=alpha(c("red", "blue"), 0.3))
```
...
#Expression patterns per cluster
pdf(file = "./figures_pdf2/Fig2B.2.pdf", width=5, height = 5)
par(mfrow=c(3, 3))
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORs, ylim=c(0, 1), ylab="fitted norm. log10 mean expression", xlab="position DV")
dev.off()
...
```{r GenesFromWeirdClusters, echo=FALSE, message=FALSE, warning=FALSE}
#Expression patterns per cluster
#for(cluster in c("pink", "greenyellow")){
#  genes=names(clusters_deORs)[which(clusters_deORs==cluster)]
#  genes=genes[1:3]
#  if(length(grep("\\.", genes))>0){gsub("\\.", "-", genes)}
#  par(mfrow=c(2, 2))
#  for(marker in genes){
#    plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position DV", ylab="rpm expression", pch=19, col=alpha(cluster, 0.3))
#  }
#}

```

**Genes from weird clusters (avg rpm vals)**

```{r GenesFromWeirdClusters_log10, echo=FALSE, message=FALSE, warning=FALSE}
#Expression patterns per cluster
#for(cluster in c("red", "green", "yellow", "black", "brown")){
#  genes=names(clusters_deORs)[which(clusters_deORs==cluster)]
#  genes=genes[1:3]
#  if(length(grep("\\.", genes))>0){gsub("\\.", "-", genes)}
#  par(mfrow=c(2, 2))
#  for(marker in genes){
#    plot(1:dim(logNormDEGs)[1], logNormDEGs[,marker], xlim=c(0, dim(logNormDEGs)[1]+1), main=marker, xlab="position LML", ylab="log10 rpm expression", pch=19, col=alpha(cluster, 0.3))
#  }
#}

```

**Genes from weird clusters log10(avg rpm vals)**

```{r GenesFromWeirdClusters_fitted, echo=FALSE, message=FALSE, warning=FALSE}
#Expression patterns per cluster
#for(cluster in c("red", "green", "yellow", "black", "brown")){
#  genes=names(clusters_deORs)[which(clusters_deORs==cluster)]
#  genes=genes[1:3]
...
#  par(mfrow=c(2, 2))
#  for(marker in genes){
#    plot(position, predict(fitted_data[[marker]],  c(1:max(position))), xlim=c(0, length(position)+1), main=marker, xlab="position LML", ylab="fitted log10 rpm expression", pch=19, col=alpha(cluster, 0.3))
#  }
#}

```

**Genes from weird clusters fitted log10(avg rpm vals)**

```{r GenesFromWeirdClusters_fittedNorm, echo=FALSE, message=FALSE, warning=FALSE}
#Expression patterns per cluster
#for(cluster in c("red", "green", "yellow", "black", "brown")){
#  genes=names(clusters_deORs)[which(clusters_deORs==cluster)]
#  genes=genes[1:3]
...
#  par(mfrow=c(2, 2))
#  for(marker in genes){
#    plot(1:dim(deORs)[2], deORs[marker,], xlim=c(0, dim(deORs)[2]+1), main=marker, xlab="position LML", ylab="rpm expression", pch=19, col=alpha(cluster, 0.3))
#  }
#}

```

**Genes from weird clusters (fitted norm vals)**
...
#plotClusterJitter(x=cluster2index$spClusterNo, y=cluster2index$spClusterMaxPos, xlabels=unique(cluster2index$Cluster[order(cluster2index$spClusterNo)]), col=cluster2index$Cluster, xlab="spatial expression cluster", ylab="Zone index")
cluster2index$modern.gene.name[which(cluster2index$myIndx==max(cluster2index$myIndx))]
```
...
#ORsDiffmap<-DiffusionMap(deORs[grep("Olfr", rownames(deORs)),])
...
#tip selection
#dorsalORs<-ORsDiffmap$DC1[names(ORsDiffmap$DC1) %in% names(clusters_DEORs)[which(clusters_DEORs=="turquoise")]]
#names(dorsalORs)[which(dorsalORs==min(dorsalORs))]
#Olfr919
#Olfr620 with euc dist

#ventralORs<-ORsDiffmap$DC2[names(ORsDiffmap$DC2) %in% names(clusters_DEORs)[which(clusters_DEORs=="red")]]
#names(ventralORs)[which(ventralORs==max(ventralORs))]
#Olfr1241
...
#dpt <- DPT(ORsDiffmap, tips=which(names(clusters_DEORs)=="Olfr78")) #pseudotime
#dpt <- DPT(ORsDiffmap, tips=which(names(clusters_DEORs)=="Olfr878")) #pseudotime
#dpt <- DPT(ORsDiffmap, tips=which(names(clusters_DEORs)=="Olfr1277")) 
#dpt <- DPT(ORsDiffmap, tips=2) #pseudotime
#dpt <- DPT(ORsDiffmap, tips=which(names(clusters_DEORs)=="Olfr919")) #pseudotime
#dpt <- DPT(ORsDiffmap, tips=which(names(clusters_DEORs)=="Olfr620")) #pseudotime
dpt <- DPT(ORsDiffmap, tips=which(names(clusters_DEORs)=="Olfr372")) #pseudotime


pdf(file = "./figures_pdf2/Fig4B.3.pdf", width=5, height = 5)
plot(ORsDiffmap$DC1, ORsDiffmap$DC2, col=alpha(plot.DPT(dpt)$data$ColourExpl, 0.3), pch=19, xlab="DC1", ylab="DC2")
dev.off()

cairo_ps("./figures_pdf2/Fig4B.3.eps", width = 5, height = 5)
plot(ORsDiffmap$DC1, ORsDiffmap$DC2, col=alpha(plot.DPT(dpt)$data$ColourExpl, 0.3), pch=19, xlab="DC1", ylab="DC2")
dev.off()
...
pdf(file = "./figures_pdf2/Fig4B.4.pdf", width=5, height = 5)
plot(x=cluster2dpt$myIndx, y=cluster2dpt$zone.index, col=alpha(cluster2dpt$Cluster, 0.3), xlab="Indexes by DPT", ylab="Zone index by Miyamichi", pch=19)
dev.off()

cairo_ps("./figures_pdf2/Fig4B.4.eps", width = 5, height = 5)
plot(x=cluster2dpt$myIndx, y=cluster2dpt$zone.index, col=alpha(cluster2dpt$Cluster, 0.3), xlab="Indexes by DPT", ylab="Zone index by Miyamichi", pch=19)
dev.off()
...
DEGsfile<-read.csv("./supp_tables/newDVDEGs.autocor.binMatFDR0.01filt_G99.csv")[,-1]
...
is.DVdeg<-sapply(neuroActMks_Up, function(x){x %in% DEGsfile$Gene.name})
DVclust<-as.character(sapply(neuroActMks_Up, function(x){DEGsfile$clustersMinCS50[which(DEGsfile$Gene.name==x)]}))

neuroActMks_Up<-data.frame(neuroActMks_Up, is.DVdeg, DVclust)
write.csv(neuroActMks_Up, "./supp_tables/neuroActMks_Up_DV.csv")

neuroActMks_Down<-c("Bace1", "Calb2", "Cdh15", "Chil3", "Efna5", "Entpd2", "Etv3", "Kirrel3", "Ppp3ca", "Rims3")
is.DVdeg<-sapply(neuroActMks_Down, function(x){x %in% DEGsfile$Gene.name})
DVclust<-as.character(sapply(neuroActMks_Down, function(x){DEGsfile$clustersMinCS50[which(DEGsfile$Gene.name==x)]}))

neuroActMks_Down<-data.frame(neuroActMks_Down, is.DVdeg, DVclust)
write.csv(neuroActMks_Down, "./supp_tables/neuroActMks_Down_DV.csv")
```
...
pdf(file = "./figures_pdf2/FigS4C_DV.pdf", width=5, height = 5)
...
cairo_ps("./figures_pdf2/FigS4C_DV.eps", width = 5, height = 5)
plot(meansLog10GenDist, meansLog10PattDist, ylim=c(0, max(meansLog10PattDist)+150), xlim=c(0, 8.5), pch=19, xlab="log10(Genomic Distance)", ylab="Pattern Distance")
arrows(meansLog10GenDist, meansLog10PattDist-sdLog10PattDist, meansLog10GenDist, meansLog10PattDist+sdLog10PattDist, length=0.05, angle=90, code=3)
set.seed(10)
abline(h=mean(sample(PatternDist, 500), na.rm=T))
...
#Eliminate Class 1 ORs
#deORs_dataRF<-deORs_dataRF[-which(deORs_dataRF$Gene.name %in% Class1$Mouse.class.I.ORs),]
...
indexRFprediction<-function(data4prediction, PredictorsSelection, trainDataFraction, predictedCol){
  ...
  #Eliminate Class 1ORs from taining dataset
  #train<-train[-which(rownames(train) %in% Class1$Mouse.class.I.ORs),]
...
```{r, RFplots}
pdf(file = "./figures_pdf2/FigS4B_DV.pdf", width=5, height = 5)
plotXYcorrelation(x=real, y=predictedRandomTraining, xlab = "real indexes", ylab="predicted indexes", col=1, main="RF predictions for OR DV indexes")
par(mar=c(8, 5, 5, 5))
barplot(rev(sort(importance[,1]))[1:10], las=2, main="predictors importance", ylab="value", cex.names = 0.6)
dev.off()

cairo_ps("./figures_pdf2/FigS4B_DV.eps", width = 5, height = 5)
plotXYcorrelation(x=real, y=predictedRandomTraining, xlab = "real indexes", ylab="predicted indexes", col=1, main="RF predictions for OR DV indexes")
par(mar=c(8, 5, 5, 5))
barplot(rev(sort(importance[,1]))[1:10], las=2, main="predictors importance", ylab="value", cex.names = 0.6)
dev.off()
...
rf.cv<-rf.crossValidation(x=rfModel[[4]], xdata=rfModel[[5]][which(rownames(rfModel[[5]]) %in% names(rfModel[[4]]$y)),])
...
pdf(file = "./figures_pdf2/Fig4D.pdf", width=5, height = 5)
par(mar=c(8, 5, 5, 5))
barplot(rev(sort(importance[,1]))[1:10], las=2, main="predictors importance", ylab="", cex.names = 0.6)
dev.off()
...
ORs_dataRF$indexMiyamichiChemS<-ORsChems$inferred.zone.index[match(rownames(ORs_dataRF), ORsChems$gene.name)]
...
olfrsCol<-rep(1, length(ORs_dataRF$indexMiyamichiChemS[which(ORs_dataRF$indexMiyamichiChemS!="low expression")]))
olfrsCol[which(rownames(ORs_dataRF)[which(ORs_dataRF$indexMiyamichiChemS!="low expression")]=="Olfr309")]<-2
olfrsCol[which(rownames(ORs_dataRF)[which(ORs_dataRF$indexMiyamichiChemS!="low expression")]=="Olfr727")]<-2

color<-as.numeric(ORs_dataRF$Class[which(ORs_dataRF$indexMiyamichiChemS!="low expression")])
color[which(color==1)]<-"red"
color[which(color==2)]<-"blue"
pdf(file = "./figures_pdf2/Fig4J.pdf", width=5, height = 5)
plotXYcorrelation(x=as.numeric(ORs_dataRF$indexMiyamichiChemS[which(ORs_dataRF$indexMiyamichiChemS!="low expression")]), y=ORs_dataRF$MiyamichiIndex[which(ORs_dataRF$indexMiyamichiChemS!="low expression")], xlab = "ChemS predicted indexes", ylab="Our predicted indexes", col=color, main="RF predictions for all OR Miyamichi indexes", ylim=c(1, 5), xlim=c(1, 5))
abline(coef = c(0,1), col="red")

points(ORs_dataRF$indexMiyamichiChemS[which(rownames(ORs_dataRF)=="Olfr309")], ORs_dataRF$MiyamichiIndex[which(rownames(ORs_dataRF)=="Olfr309")], pch=19, col=alpha("green", 0.6))
points(ORs_dataRF$indexMiyamichiChemS[which(rownames(ORs_dataRF)=="Olfr727")], ORs_dataRF$MiyamichiIndex[which(rownames(ORs_dataRF)=="Olfr727")], pch=19, col=alpha("green", 0.6))

dev.off()

cairo_ps("./figures_pdf2/Fig4J.eps", width = 5, height = 5)
plotXYcorrelation(x=as.numeric(ORs_dataRF$indexMiyamichiChemS[which(ORs_dataRF$indexMiyamichiChemS!="low expression")]), y=ORs_dataRF$MiyamichiIndex[which(ORs_dataRF$indexMiyamichiChemS!="low expression")], xlab = "ChemS predicted indexes", ylab="Our predicted indexes", col=1, main="RF predictions for all OR Miyamichi indexes", ylim=c(1, 5), xlim=c(1, 5))
dev.off()

color<-as.numeric(ORs_dataRF$Class[which(is.na(ORs_dataRF$index) & !is.na(ORs_dataRF$indexMiyamichiReal))])
color[which(color==2)]<-"blue"
color[which(color!="blue")]<-"red"
pdf(file = "./figures_pdf2/Fig4F.pdf", width=5, height = 5)
plotXYcorrelation(y=ORs_dataRF$indexMiyamichiReal[which(is.na(ORs_dataRF$index) & !is.na(ORs_dataRF$indexMiyamichiReal))], x=ORs_dataRF$MiyamichiIndex[which(is.na(ORs_dataRF$index) & !is.na(ORs_dataRF$indexMiyamichiReal))], ylab = "real indexes", xlab="predicted indexes", col=color, main="predicted Miy. indexes for lowly expressed ORs with real Miy.index", ylim=c(1, 5), xlim=c(1, 5))
abline(coef = c(0,1), col="red")
dev.off()

cairo_ps("./figures_pdf2/Fig4F.eps", width = 5, height = 5)
...
```{r MombaertsZolfrs}
Zolfrs<-read.xls("./mombaertsZones.xlsx", stringsAsFactors=F)[,1:2]

ORs_dataRF$ZolfrReal<-Zolfrs$ZOLFR[match(rownames(ORs_dataRF),Zolfrs$Olfr)]
ORs_dataRF$ZolfrNoReal<-NA

#Order Mombaerts zones
ZolfrMeans<-tapply(ORs_dataRF$DPTindex, ORs_dataRF$ZolfrReal, function(x){mean(x, na.rm=T)})
ZolfrMeans<-sort(ZolfrMeans)
# order is almost same as in the paper -> c("Zolfr160", "Zolfr24", "Zolfr17", "Zolfr155", "Zolfr374", "Zolfr2", "Zolfr1507", "Zolfr15", "Zolfr653")
count=1
for(zone in names(ZolfrMeans)){
  ORs_dataRF$ZolfrNoReal[which(ORs_dataRF$ZolfrReal==zone)]<-count
  count<-count+1
}
rm(count)

#mod<-lm(indexMiyamichiReal ~ DPTindex, data=ORs_dataRF[which(!is.na(ORs_dataRF$indexMiyamichiReal)),])
mod<-gam(ZolfrNoReal ~ s(DPTindex), data=ORs_dataRF[which(!is.na(ORs_dataRF$ZolfrNoReal)),])
summary(mod)
ORs_dataRF$MombaertsZolfr<-predict(mod, ORs_dataRF)


plotXYcorrelation(x=ORs_dataRF$ZolfrNoReal[which(!is.na(ORs_dataRF$ZolfrNoReal))], y=ORs_dataRF$MombaertsZolfr[which(!is.na(ORs_dataRF$ZolfrNoReal))], xlab = "real indexes", ylab="predicted indexes", col=1, main="RF predictions for all Mombaerts Zolfrs", ylim=c(1, 9), xlim=c(1, 9))

pdf(file = "./figures_pdf2/Fig4G.pdf", width=5, height = 5)
plotXYcorrelation(y=ORs_dataRF$ZolfrNoReal[which(is.na(ORs_dataRF$index) & !is.na(ORs_dataRF$ZolfrNoReal))], x=ORs_dataRF$MombaertsZolfr[which(is.na(ORs_dataRF$index) & !is.na(ORs_dataRF$ZolfrNoReal))], ylab = "real indexes", xlab="predicted indexes", col=1, main="predicted Mombaerts Zolfrs for lowly expressed ORs with real Zolfr", ylim=c(1, 9), xlim=c(1, 9))
abline(coef = c(0,1), col="red")
dev.off()

cairo_ps("./figures_pdf2/Fig4G.eps", width = 5, height = 5)
plotXYcorrelation(y=ORs_dataRF$ZolfrNoReal[which(is.na(ORs_dataRF$index) & !is.na(ORs_dataRF$ZolfrNoReal))], x=ORs_dataRF$MombaertsZolfr[which(is.na(ORs_dataRF$index) & !is.na(ORs_dataRF$ZolfrNoReal))], ylab = "real indexes", xlab="predicted indexes", col=1, main="predicted Mombaerts Zolfrs for lowly expressed ORs with real Zolfr", ylim=c(1, 9), xlim=c(1, 9))
dev.off()
```

**Predictions for mombaerts zones**
...
pdf(file = "./figures_pdf2/Fig4E.pdf", width=5, height = 5)
plot(ORs_dataRF$DPTindex, MiyIndex, pch=19, col=alpha(color, 0.3))
points(ORs_dataRF$DPTindex[which(rownames(ORs_dataRF)=="Olfr309")], MiyIndex[which(rownames(ORs_dataRF)=="Olfr309")], pch=19, col=alpha("green", 0.6))
points(ORs_dataRF$DPTindex[which(rownames(ORs_dataRF)=="Olfr727")], MiyIndex[which(rownames(ORs_dataRF)=="Olfr727")], pch=19, col=alpha("green", 0.6))
dev.off()

cairo_ps("./figures_pdf2/Fig4E.eps", width = 5, height = 5)
plot(ORs_dataRF$DPTindex, MiyIndex, pch=19, col=alpha(color, 0.3))
dev.off()

cor.test(ORs_dataRF$DPTindex, MiyIndex, method="spearman")

Zolfr<-ORs_dataRF$MombaertsZolfr
Zolfr[which(!is.na(ORs_dataRF$ZolfrNoReal))]<-ORs_dataRF$ZolfrNoReal[which(!is.na(ORs_dataRF$ZolfrNoReal))]

color=rep("black", length(Zolfr))
color[which(!is.na(ORs_dataRF$ZolfrNoReal))]<-"red"

pdf(file = "./figures_pdf2/Fig4H.pdf", width=5, height = 5)
plot(ORs_dataRF$DPTindex, Zolfr, pch=19, col=alpha(color, 0.3))
points(ORs_dataRF$DPTindex[which(rownames(ORs_dataRF)=="Olfr309")], Zolfr[which(rownames(ORs_dataRF)=="Olfr309")], pch=19, col=alpha("green", 0.6))
points(ORs_dataRF$DPTindex[which(rownames(ORs_dataRF)=="Olfr727")], Zolfr[which(rownames(ORs_dataRF)=="Olfr727")], pch=19, col=alpha("green", 0.6))
dev.off()

cairo_ps("./figures_pdf2/Fig4H.eps", width = 5, height = 5)
plot(ORs_dataRF$DPTindex, Zolfr, pch=19, col=alpha(color, 0.3))
dev.off()

cor.test(ORs_dataRF$DPTindex, Zolfr, method="spearman")
write.csv(ORs_dataRF, "./supp_tables/allORs_RealAndPredictedIndexes.csv")

## MIy vs Momb

color=rep("black", length(Zolfr))
color[which((is.na(ORs_dataRF$ZolfrNoReal)) & (!is.na(ORs_dataRF$indexMiyamichiReal)))]<-"red"
color[which((!is.na(ORs_dataRF$ZolfrNoReal)) & (is.na(ORs_dataRF$indexMiyamichiReal)))]<-"blue"
color[which((!is.na(ORs_dataRF$ZolfrNoReal)) & (!is.na(ORs_dataRF$indexMiyamichiReal)))]<-"green"

pdf(file = "./figures_pdf2/Fig4I.pdf", width=5, height = 5)
plot(MiyIndex, Zolfr, pch=19, col=alpha(color, 0.3))
legend("bottomright", c("With Real MI", "With Real Zolfr", "With both Real indexes", "No real index", "validated ORs"), pch=19, col=alpha(c("red", "blue", "green", "black", "orange"), 0.3), cex=0.8)
points(MiyIndex[which(rownames(ORs_dataRF)=="Olfr309")], Zolfr[which(rownames(ORs_dataRF)=="Olfr309")], pch=19, col=alpha("orange", 0.6))
points(MiyIndex[which(rownames(ORs_dataRF)=="Olfr727")], Zolfr[which(rownames(ORs_dataRF)=="Olfr727")], pch=19, col=alpha("orange", 0.6))
dev.off()

cairo_ps("./figures_pdf2/Fig4I.eps", width = 5, height = 5)
plot(MiyIndex, Zolfr, pch=19, col=alpha(color, 0.3))
legend("bottomright", c("With Real MI", "With Real Zolfr", "With both Real indexes", "No real index"), pch=19, col=alpha(c("red", "blue", "green", "black"), 0.3), cex=0.8)
dev.off()

cor.test(MiyIndex, Zolfr, method="spearman")
```

**DPT vs Miyamichi index** red=real Miyamichi index, black = predicted Miyamichi indexes

```{r OBquads, echo=FALSE, message=FALSE, warning=FALSE}
DoBs<-read.csv("./supp_tables/ORsDegreesOfBelonging.csv", row.names=1)
DoBsOr<-DoBs

DoBs<-DoBs[,-dim(DoBs)[2]]
DoBs2<-DoBs
rownames(DoBs2)<-paste(rownames(DoBs2), "-2", sep="")
DoBs<-rbind(DoBs, DoBs2)

quads<-read.csv("./supp_tables/goodpoints_quadrants.csv")

## Dist of ORs with a maximum topic in quads
par(mfrow=c(2, 3))
for(topic in sort(unique(DoBsOr$maxTopic))){
  dist<-c(length(which(quads$olfrname[quads$dim=="Dorsal"] %in% rownames(DoBsOr)[DoBsOr$maxTopic==topic])),
          length(which(quads$olfrname[quads$dim=="Ventral"] %in% rownames(DoBsOr)[DoBsOr$maxTopic==topic])),
          length(which(quads$olfrname[quads$dim=="Medial"] %in% rownames(DoBsOr)[DoBsOr$maxTopic==topic])),
          length(which(quads$olfrname[quads$dim=="Lateral"] %in% rownames(DoBsOr)[DoBsOr$maxTopic==topic])))
  
  names(dist)<-c("Dorsal", "Ventral", "Medial", "Lateral")
  
  barplot(dist, main=paste("ORs with peak in Topic", topic), xlab="", ylab="No. of ORs", las=2)
}

## Previously known Miy indexes
par(mfrow=c(2, 3))
MiyIndex4BP<-ORs_dataRF$indexMiyamichiReal[!is.na(ORs_dataRF$indexMiyamichiReal)]
names(MiyIndex4BP)<-rownames(ORs_dataRF)[!is.na(ORs_dataRF$indexMiyamichiReal)]

DorsalQ<-MiyIndex4BP[which(names(MiyIndex4BP) %in% quads$olfrname[quads$dim=="Dorsal"])]
MedialQ<-MiyIndex4BP[which(names(MiyIndex4BP) %in% quads$olfrname[quads$dim=="Medial"])]
VentralQ<-MiyIndex4BP[which(names(MiyIndex4BP) %in% quads$olfrname[quads$dim=="Ventral"])]
LateralQ<-MiyIndex4BP[which(names(MiyIndex4BP) %in% quads$olfrname[quads$dim=="Lateral"])]

boxplot(DorsalQ, MedialQ, LateralQ, VentralQ, names=c("DorsalQ", "MedialQ", "LateralQ", "VentralQ"), main="Miyamichi Indexes in OB quads.", las=2)

## Zolfrs

Zolfrs4BP<-ORs_dataRF$ZolfrNoReal[!is.na(ORs_dataRF$ZolfrNoReal)]
names(Zolfrs4BP)<-rownames(ORs_dataRF)[!is.na(ORs_dataRF$ZolfrNoReal)]

DorsalQ<-Zolfrs4BP[which(names(Zolfrs4BP) %in% quads$olfrname[quads$dim=="Dorsal"])]
MedialQ<-Zolfrs4BP[which(names(Zolfrs4BP) %in% quads$olfrname[quads$dim=="Medial"])]
VentralQ<-Zolfrs4BP[which(names(Zolfrs4BP) %in% quads$olfrname[quads$dim=="Ventral"])]
LateralQ<-Zolfrs4BP[which(names(Zolfrs4BP) %in% quads$olfrname[quads$dim=="Lateral"])]

boxplot(DorsalQ, MedialQ, LateralQ, VentralQ, names=c("DorsalQ", "MedialQ", "LateralQ", "VentralQ"), main="Mombaerts Zolfrs in OB quads.", las=2)

## Our DPT indexes

DorsalQ<-ORs_dataRF$DPTindex[which(rownames(ORs_dataRF) %in% quads$olfrname[quads$dim=="Dorsal"])]
MedialQ<-ORs_dataRF$DPTindex[which(rownames(ORs_dataRF) %in% quads$olfrname[quads$dim=="Medial"])]
VentralQ<-ORs_dataRF$DPTindex[which(rownames(ORs_dataRF) %in% quads$olfrname[quads$dim=="Ventral"])]
LateralQ<-ORs_dataRF$DPTindex[which(rownames(ORs_dataRF) %in% quads$olfrname[quads$dim=="Lateral"])]

boxplot(DorsalQ, MedialQ, LateralQ, VentralQ, names=c("DorsalQ", "MedialQ", "LateralQ", "VentralQ"), main="DV DPT Indexes in OB quads.", las=2)

#plotXYcorrelation(x=ORs_dataRF$dist2ClosestIsland, y=ORs_dataRF$Gene.start..bp., xlab = "dist2ClosestIsland", ylab="Gene start pos.", col=1, main="gene start and island dist")

#plotXYcorrelation(x=ORs_dataRF$dist2ClosestIsland, y=ORs_dataRF$Gene.end..bp., xlab = "dist2ClosestIsland", ylab="Gene end pos.", col=1, main="gene end and island dist")
##

quads$olfrname[seq(2, dim(quads)[1], by = 2)]<-paste(quads$olfrname[seq(2, dim(quads)[1], by = 2)], "-2", sep="")

ORsQuad<-quads$dim[match(rownames(DoBs), quads$olfrname)]
ORsQuadClust<-ORsQuad
ORsQuadClust[which(ORsQuadClust=="Dorsal")]<-"turquoise"
ORsQuadClust[which(ORsQuadClust=="Ventral")]<-"orange"
ORsQuadClust[which(ORsQuadClust=="Medial")]<-"magenta"
ORsQuadClust[which(ORsQuadClust=="Lateral")]<-"yellow"

names(ORsQuadClust)<-rownames(DoBs)

DoBs<-DoBs[-which(is.na(ORsQuadClust)),]
ORsQuadClust<-ORsQuadClust[-which(is.na(ORsQuadClust))]

par(mfrow=c(2, 2))
plotMeanPatternPerCluster(data=as.matrix(DoBs), clusters=ORsQuadClust, ylim=c(0, 1), ylab="degree of belonging", xlab="topic")
...
for(cluster in unique(DEGsfile$clustersMinCS25)){
  ...
  write.table(tmp, paste("./supp_tables/forGOanalysis/", cluster, "_newDVDEGs_minCS25.csv", sep=""), row.names = F, quote = F, col.names = F)
}
...
for(cluster in unique(DEGsfile$clustersMinCS50)){
  ...
  write.table(tmp, paste("./supp_tables/forGOanalysis/", cluster, "_newDVDEGs_minCS50.csv", sep=""), row.names = F, quote = F, col.names = F)
}

for(cluster in unique(DEGsfile$clustersMinCS100)){
  tmp=DEGsfile$Gene.name[which(DEGsfile$clustersMinCS100==cluster)]
  write.table(tmp, paste("./supp_tables/forGOanalysis/", cluster, "_newDVDEGs_minCS100.csv", sep=""), row.names = F, quote = F, col.names = F)
}

tmp=rownames(normalized)[which(filt==1)]
write.table(tmp, "./supp_tables/forGOanalysis/newDVgenes_GObg_ORs.csv", row.names = F, quote = F, col.names = F)

tmp=rownames(normalized)[which(filt==1)]
tmp=tmp[-grep("Olfr", tmp)]
write.table(tmp, "./supp_tables/forGOanalysis/newDVgenes_GObg.csv", row.names = F, quote = F, col.names = F)
```

```{r GOplotsMinCS50, echo=FALSE, message=FALSE, warning=FALSE}

pdf(file = "./figures_pdf2/FigS2A_DV.pdf", width=7, height = 5)

par(las=2, mar=c(6,21,4,3))

for(file in paste("./supp_tables/forGOanalysis/gorilla_GO/", list.files(path="./supp_tables/forGOanalysis/gorilla_GO/", pattern = "GO_DV.+MinCS50"), sep="")){
  ...
  plotTitle<-gsub("./supp_tables/forGOanalysis/gorilla_GO/GO_DV", "", file)
...
#for(file in paste("./supp_tables/forGOanalysis/gorilla_GO/", list.files(path="./supp_tables/forGOanalysis/gorilla_GO/", pattern = "GO_DV.+MinCS100"), sep="")){
#  GSEA<-read.csv(file, stringsAsFactors = F, sep="\t")
#  GSEA<-GSEA[grep("GO:", GSEA$GO.Term),1:5]
#  GSEA<-GSEA[which(GSEA$FDR.q.value<0.05),]
#  plotTitle<-gsub("./supp_tables/forGOanalysis/gorilla_GO/GO_DV", "", file)
...
write.csv(normalized, "./supp_tables/newDV_nonORS_normalized_rpm_repsAvg_G99.csv")
...
write.csv(DEGsfile, "./supp_tables/newDVDEnonORs.autocor.binMatFDR0.01filt_G99.csv")
...
DEGsfile<-cbind(DEGsfile, clustersMinCS50=clusters_deORs, clustersMinCS100=clusters_deORsMinCS100)
write.csv(DEGsfile, "./supp_tables/newDVDEnonORs.autocor.binMatFDR0.01filt_G99.csv")
