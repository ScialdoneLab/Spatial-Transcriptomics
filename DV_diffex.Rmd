...
DEGs<-read.csv("./DEGs_expression.csv", header = TRUE, row.names = 1, stringsAsFactors = F)
resOrdered<-read.csv("./diffexp_windows10.csv", header=TRUE, row.names=1, stringsAsFactors = F)
normalized<-read.csv("./DVnormalized.csv", header=TRUE, row.names=1, stringsAsFactors = F)
min_max<-read.csv("./minmax.csv", header=TRUE, row.names=1, stringsAsFactors = F)

#Spatial expression patterns of differentially expressed ORs
...
heatmap.2(heatDEORs, Colv = NA, xlab="Position D-V", trace='none', key.title="log10 gene expression", key.ylab=NA, key.xlab=NA, col=my_palette, cexRow = 0.8, cexCol = 0.8, Rowv = F)

#Spatial expression patterns of non differentially expressed ORs
...
heatmap.2(heatNdeORs, Colv = NA, xlab="Position D-V", trace='none', key.title="log10 gene expression", key.ylab=NA, key.xlab=NA, col=my_palette, cexRow = 0.8, cexCol = 0.8, Rowv = F)

#Spatial expression patterns of OSN markers
OSN<-as.data.frame(normalized[rownames(normalized)=="Omp",])
for(marker in c("Gnal", "Cnga2")){
  OSN<-cbind(OSN, normalized[rownames(normalized)==marker,])
}
colnames(OSN)<-c("Omp", "Gnal", "Cnga2")

heatOSN<-t(log10(OSN+1))
colnames(heatOSN)<-normalized[dim(normalized)[1],]
heatmap.2(heatOSN, Colv = NA, xlab="Position D-V", trace='none', key.title="log10 gene expression", key.ylab=NA, key.xlab=NA, col=my_palette, cexRow = 0.8, cexCol = 0.8, Rowv = F)
```

Figure 17. Spatial expression patterns of ORs. a) Differentially expressed ORs. b) Non-differentially expressed ORs

```{r}
#Clustering by differentially expressed ORs
...
position<-sub('\\..*', '', colnames(deORs))
position<-as.numeric(gsub("X", "", position))
norm_deORs<-log10(deORs+1)
norm_deORs<-apply(norm_deORs, 1, function(x){(x-min(x))/(max(x)-min(x))})
norm_deORs<-data.frame(position, norm_deORs)
#norm_degs<-t(norm_degs)

library(locfit)

fitted_data<-apply(norm_deORs[,2:length(colnames(norm_deORs))], 2, function(x){locfit(x~lp(norm_deORs[,1], deg = 2, nn=1), data=norm_deORs)})
...
for(i in unique(clusters_deORs)){
  means<-rowMeans(norm_deORs[,-1][,which(clusters_deORs==i)])
  plot(locfit(means~lp(norm_deORs[,1], deg=2, nn=1)), ylim=c(0, 1), main=i, ylab="norm. log10 expression", xlab="position D-V")
#  plot(fitted_data[[rownames(DEGs)[which(clusters==i)][1]]], ylim=c(0, 1), main=i, ylab="norm. log10 expression", type="l", col=i, xlab="position D-V")
  sapply(rownames(deORs)[which(clusters_deORs==i)], function(x){lines(fitted_data[[x]], col=alpha(i, 0.1))})
  lines(locfit(means~lp(norm_deORs[,1], deg=2, nn=1)), lwd=2)
}
