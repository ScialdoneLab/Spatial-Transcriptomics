...
genes_data<-read.csv("./mm10_gene_coords.txt", stringsAsFactors = F, header=T)
...
#write.csv(normalized, "./DVnormalized.csv")
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
normalized<-read.csv("./DVnormalized.csv", header=TRUE, row.names=1, stringsAsFactors = F)
...
min_max<-read.csv("./minmax.csv", header=TRUE, stringsAsFactors = F)
min_max<-data.matrix(min_max)
rownames(min_max)<-rownames(selected)
min_max<-min_max[,-1]
...
dds1 <- DESeqDataSetFromMatrix(countData=min_max, colData=coldata, design=~group) #transform dataset
dds1<-DESeq(dds1)
res<-results(dds1)
...
heatmap.2(as.matrix(heatDEGs), Colv = NA, xlab="Position D-V", trace='none', key.title="log10 gene expression", key.ylab=NA, key.xlab=NA, col=my_palette, cexRow = 0.8, cexCol = 0.8)
...
heatmap.2(as.matrix(heatDEGs2), Colv = NA, xlab="Position D-V", trace='none', key.title="log10 gene expression", key.ylab=NA, key.xlab=NA, col=my_palette, cexRow = 0.8, cexCol = 0.8)
```
...
```{r}
#Expression patterns of known genes with specific spatial patterns
...
plot(norm_degs[,1], norm_degs[,gene], main="Acsm4", xlab="position D-V", ylab="normalized log10 expression")
lines(locfit(norm_degs[,gene]~lp(norm_degs[,1], deg = 2, nn=1), data=norm_degs), col="red")

gene<-which(colnames(norm_degs)=="Olfr727")
plot(norm_degs[,1], norm_degs[,gene], main="Olfr727", xlab="position D-V", ylab="normalized log10 expression")
lines(locfit(norm_degs[,gene]~lp(norm_degs[,1], deg = 2, nn=1), data=norm_degs), col="red")
```

Figure 15. Spatial expression patterns of genes with known patterns. a) Acsm4, b) Olfr727 
...
#look at not differentially expressed ORs

heatDEORs<-log10(normalized[match(rownames(resOrdered)[grep("Olfr", rownames(resOrdered)[resOrdered$padj<0.05])], rownames(normalized)),]+1)
colnames(heatDEORs)<-normalized[dim(normalized)[1],]
heatmap.2(as.matrix(heatDEORs), Colv = NA, xlab="Position D-V", trace='none', key.title="log10 gene expression", key.ylab=NA, key.xlab=NA, col=my_palette, cexRow = 0.8, cexCol = 0.8, Rowv = F)

heatNdeORs<-log10(normalized[match(rownames(resOrdered)[grep("Olfr", rownames(resOrdered)[resOrdered$padj>=0.05])], rownames(normalized)),]+1)
colnames(heatNdeORs)<-normalized[dim(normalized)[1],]
heatmap.2(as.matrix(heatNdeORs), Colv = NA, xlab="Position D-V", trace='none', key.title="log10 gene expression", key.ylab=NA, key.xlab=NA, col=my_palette, cexRow = 0.8, cexCol = 0.8, Rowv = F)
```

Figure 17. Spatial expression patterns of ORs. a) Differentially expressed ORs. b) Non-differentially expressed ORs

```{r}
#Clustering by fitted data from differentially expressed ORs 

#deORs<-DEGs[grep("Olfr", rownames(DEGs)),]
deORs<-fitted_data[grep("Olfr", names(fitted_data))]
deORs<-sapply(1:length(deORs), function(x){predict(deORs[[x]],  c(1:72))})
colnames(deORs)<-names(fitted_data[grep("Olfr", names(fitted_data))])
deORs<-t(deORs)

cor.deORs<-cor(t(deORs),method="spearman") #compute correlation matrix
test.dist<-as.dist(sqrt(0.5*((1-cor.deORs)))) #define distance matrix
...
set.seed(10)
test.tsne<-Rtsne(test.dist, is_distance = T)
plot(test.tsne$Y, col=clusters_deORs)
```

Figure 18. Tsne plot for differentially expressed ORs. Colors = clusters

```{r}
#Expression patterns per cluster

library(matrixStats)
position<-1:dim(deORs)[2]
...
for(i in unique(clusters_deORs)){
  means<-colMeans(deORs[which(clusters_deORs==i),])
  plot(means, ylim=c(0, 1), main=i, ylab="fitted norm. log10 expression", xlab="position D-V")
#  plot(fitted_data[[rownames(DEGs)[which(clusters==i)][1]]], ylim=c(0, 1), main=i, ylab="norm. log10 expression", type="l", col=i, xlab="position D-V")
  sapply(rownames(deORs)[which(clusters_deORs==i)], function(x){lines(deORs[x,], col=alpha(i, 0.1))})
  lines(means, lwd=2)
}

```

Figure 19. Average gene expression per cluster across the DV axis and spatial expression pattern per gene in colors. Gene expression values were log10 scaled and then normalized to values between 0 and 1.
...
clust<-character()
for(j in unique(ORs_data$Chromosome.scaffold.name)){
  sel<-which(ORs_data$Chromosome.scaffold.name==j)
  ORs_data[sel,]<-ORs_data[sel,][order(ORs_data[sel,]$Gene.start..bp.),]
  ...
  thr<-300000
  while(i<=length(sel)){
    if(ORs_data$Gene.start..bp.[sel[i]]<=ORs_data$Gene.start..bp.[sel[i-1]]+thr){
