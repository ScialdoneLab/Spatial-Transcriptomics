...
ui<-secure_app(head_auth = tags$script(inactivity), fluidPage( #App structure, where you define the title, the buttons or searchers and declare the outputs (plots) that will appear
  ...
  tabsetPanel(type = "tabs",
    ...
    tabPanel("Main",
      ...
      fluidRow(wellPanel("Mouse Main Olfactory Epithelium Atlas", style = "background-color:coral;font-size:40px")),
    ...
    tabPanel("3D Topics",
      ...
      fluidRow(wellPanel("3D Zones (Topics)", style = "background-color:gold;font-size:20px")),
  
      fluidRow(wellPanel("", style = "background-color:white", actionButton("topics3D", "see 3D topics"))),
