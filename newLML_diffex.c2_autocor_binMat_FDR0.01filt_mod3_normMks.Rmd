---
title: "LML differential expression"
...
#LML_dataset<-read.csv("./LML_dataset_G99.csv", stringsAsFactors = FALSE, sep="\t", row.names = 1, header=FALSE)

#colnames(LML_dataset)<-paste(LML_dataset[1,], 1:dim(LML_dataset)[2], sep="")
#LML_dataset<-LML_dataset[,-grep("0610005C13Rik", colnames(LML_dataset))]

#LML_names<-read.csv("./LMLsample_names_G99.csv", header=FALSE, stringsAsFactors = FALSE)

#LML_names<-LML_names[,1]
#LML_names<-gsub(".counts.csv", "", LML_names)
#colnames(LML_dataset)<-LML_names

#write.csv(LML_dataset, "./LML_datasetNew_G99.csv")
```
...
```{r}
#cellPropsNusvr<-read.csv("./supp_tables/cellTypesProps_nusvr_LML.csv", row.names=1)
cellPropsNusvr<-read.csv("./supp_tables/cellTypesProps_nusvr_T2_LML.csv", row.names=1)
...
#cellPropsNnls<-read.csv("./supp_tables/cellTypesProps_nnls_LML.csv", row.names=1)
cellPropsNnls<-read.csv("./supp_tables/cellTypesProps_nnls_T2_LML.csv", row.names=1)
...
#pdf(file = "./figures_pdf2/FigCellDeconvNNLS_LML_rev.pdf", width=5, height = 5)
pdf(file = "./figures_pdf2/FigCellDeconvNNLS_T2_LML_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNnls)){
  plot(1:dim(cellPropsNnls)[1], cellPropsNnls[,cell], xlim=c(0, dim(cellPropsNnls)[1]), main=paste("Nnls", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, 0.5))
  lines(locfit(cellPropsNnls[,cell]~lp(1:dim(cellPropsNnls)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNnls)), col="red")
}

par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNnls)){
  plot(1:dim(cellPropsNnls)[1], cellPropsNnls[,cell], xlim=c(0, dim(cellPropsNnls)[1]), main=paste("Nnls", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, max(cellPropsNnls[,cell])))
  lines(locfit(cellPropsNnls[,cell]~lp(1:dim(cellPropsNnls)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNnls)), col="red")
}
dev.off()

#pdf(file = "./figures_pdf2/FigCellDeconvNuSVR_LML_rev.pdf", width=5, height = 5)
pdf(file = "./figures_pdf2/FigCellDeconvNuSVR_T2_LML_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNusvr)){
  plot(1:dim(cellPropsNusvr)[1], cellPropsNusvr[,cell], xlim=c(0, dim(cellPropsNusvr)[1]), main=paste("Nusvr", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, 0.5))
  lines(locfit(cellPropsNusvr[,cell]~lp(1:dim(cellPropsNusvr)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNusvr)), col="red")
}

par(mfrow=c(2, 2))
for(cell in colnames(cellPropsNusvr)){
  plot(1:dim(cellPropsNusvr)[1], cellPropsNusvr[,cell], xlim=c(0, dim(cellPropsNusvr)[1]), main=paste("Nusvr", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, max(cellPropsNusvr[,cell])))
  lines(locfit(cellPropsNusvr[,cell]~lp(1:dim(cellPropsNusvr)[1], deg = 2, nn=1), data=as.data.frame(cellPropsNusvr)), col="red")
}
dev.off()
...
#write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_NusvrLML.csv")
write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_Nusvr_T2_LML.csv")

binMat_CT<-binarizeData(data=t(cellPropsNnls))
CT_table<-autocorrelationTest(data=binMat_CT, maxLag=(dim(binMat_CT)[2]-10), filter=rep(1, dim(binMat_CT)[1]))
#write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_NnlsLML.csv")
write.csv(CT_table, "./supp_tables/SDEtest_cellTypeProps_Nnls_T2_LML.csv")

pdf(file = "./figures_pdf2/FigCellDeconvNuSVR_HM_T2_LM_rev.pdf", width=5, height = 5)
...
totalReads<-read.csv("./LML_totalReads_G99.csv", header=FALSE, stringsAsFactors = FALSE)
...
expr<-as.matrix(LML_dataset[1:(dim(LML_dataset)[1]-5),])
stats<-LML_dataset[(dim(LML_dataset)[1]-4):dim(LML_dataset)[1],]
...
col[60:124]<-"blue" #LML2
col[125:190]<-"green" #LML3
...
plotQCstats(statsData = stats, rows = c(8:12), x=as.numeric(log10(stats[7,]+1)), xlab="log10(total reads+1)", col=col, legend_names=c("LML1", "LML2", "LML3"), legend_colors=c("red", "blue", "green"), ycut = ythresholds, xcut = xthresholds)
```
...
pdf(file = "./figures_pdf2/FigS1E_LML.pdf", width=5, height = 5)
...
#write.csv(marker_heat, "./supp_tables/log10NeuronalMksLML.csv")
```
...
par(mfrow=c(3, 2), mar=c(4,2,2,2))
plotQCstats(statsData = stats, rows = c(8:12), x=as.numeric(log10(stats[7,]+1)), xlab="log10(total reads+1)", col=col, legend_names=c("Not Selected", "Selected_LML1", "Selected_LML2", "Selected_LML3"), legend_colors=c("black", "red", "blue", "green"), ycut = ythresholds, xcut = xthresholds)
...
pdf(file = "./figures_pdf2/FigS1A_LML.pdf", width=5, height = 5)
boxplot(as.numeric(stats[8, selection]), main=rownames(stats)[8], ylim=c(40, 100), xlab="LML axis")
dev.off()

pdf(file = "./figures_pdf2/FigS1C_LML.pdf", width=5, height = 5)
boxplot(as.numeric(log10(stats[11, selection]+1)), main=paste("log10", rownames(stats)[11], sep=""), ylim=c(2,5), xlab="LML axis")
dev.off()

pdf(file = "./figures_pdf2/FigS1B_LML.pdf", width=5, height = 5)
boxplot(as.numeric(stats[12, selection]), main=rownames(stats)[12], ylim=c(0, 20), xlab="LML axis")
dev.off()

pdf(file = "./figures_pdf2/FigS1D_LML.pdf", width=5, height = 5)
boxplot(as.numeric(log10(stats[7, selection]+1)), main="Total no. of reads (log10)", ylim=c(4.5, 8), xlab="LML axis")
dev.off()
...
#write.csv(stats, "./supp_tables/newLML_QCstats_G99.csv")
#write.csv(stats[,selection], "./supp_tables/newLML_selected_QCstats_G99.csv")

position<-c(1:59, 1:65, 1:66)
sel_positions<-position[selection]
selected<-expr[, selection] #select samples with nice quality stats

batch<-col[which(col!="black")]
batch[which(batch=="red")]<-"LML1"
batch[which(batch=="blue")]<-"LML2"
batch[which(batch=="green")]<-"LML3"
rawDS<-selected
colnames(rawDS)<-paste(batch, sel_positions, sep="-")
#write.csv(rawDS, "./supp_tables/newLML_rawQCdone_G99.csv")
rm(rawDS)
...
#write.csv(normalized, "./newLMLnormalized.rpmJust-0s_G99.csv")

batch<-col[which(col!="black")]
batch[which(batch=="red")]<-"LML1"
batch[which(batch=="blue")]<-"LML2"
batch[which(batch=="green")]<-"LML3"
#write.csv(normalized[, which(batch=="LML1")], "./supp_tables/newLMLnormalized_b1.rpmJust-0s_G99.csv")
#write.csv(normalized[, which(batch=="LML2")], "./supp_tables/newLMLnormalized_b2.rpmJust-0s_G99.csv")
#write.csv(normalized[, which(batch=="LML3")], "./supp_tables/newLMLnormalized_b3.rpmJust-0s_G99.csv")
...
initSamplesPerBatch<-c(59, 65, 66) #initial number of samples we had per batch
names(initSamplesPerBatch)<-c("LML1", "LML2", "LML3")

#write.csv(getSamplesPerBatch(batchVector = batch, totalSamplesPerBatch = initSamplesPerBatch), "./supp_tables/LMLgoodSamplesPerBatch.csv")
...
oneRepGenes<-data.frame(b1=rowSums(normalized[,which(batch=="LML1")])>1, b2=rowSums(normalized[,which(batch=="LML2")])>1, b3=rowSums(normalized[,which(batch=="LML3")])>1)
...
HVGsB1<-rownames(getHVGs(data=normalized[,which(batch=="LML1")]))[1:3000]
HVGsB2<-rownames(getHVGs(data=normalized[,which(batch=="LML2")]))[1:3000]
HVGsB3<-rownames(getHVGs(data=normalized[,which(batch=="LML3")]))[1:3000]
...
for(i in 8:12){
  plotXYcorrelation(x=sel_positions, y=as.numeric(stats[i,selection]), main=c(paste("LML position vs.", rownames(stats)[i])), ylab=rownames(stats)[i], xlab="LML position", col=color_selection[-which(color_selection=="black")])
}

plotXYcorrelation(x=sel_positions, y=as.numeric(log10(stats[7, selection]+1)), main="LML position vs. Total no. of reads (log10)", ylab="Total no. of reads (log10)", xlab="LML position", col=color_selection[-which(color_selection=="black")])
...
plotXYcorrelation(x=samples_pcomp2$x[,1], y=sel_positions, main="PC1 vs LML position", ylab="LML position", xlab="PC1", col=color_selection[-which(color_selection=="black")])

plotXYcorrelation(x=samples_pcomp2$x[,2], y=sel_positions, main="PC2 vs LML position", ylab="LML position", xlab="PC2", col=color_selection[-which(color_selection=="black")])
```
...
heatmap.2(as.matrix(marker_heat), xlab="Position LML", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, key.xlab = "log10geneExp", Colv=F)
...
par(mfrow=c(2, 2))
plotGenesInEachBatch(data=log10(normalized+1), genes=c("Cnga2", "Epas1", "Ric8b", "Rtp1", "Rtp2", "Cnga4", "Gap43", "Omp", "Gng13", "Gnal", "Gnb1", "Gnai2", "Gnao1", "Six1", "Alcam", "Ano2", "Adcy3", "Chl1", "Trp63", "Slc2a3", "Neurog1", "Plcb2", "Ascl1", "Trpc2", "Reep1", "Krt5", "Sema3a", "Nrp1", "Olfr1507", "Olfr449"), x=sel_positions, batches=c("LML1", "LML2", "LML3"), batchColors=c("red", "blue", "green"), xlab="position LML", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))

par(mfrow=c(2, 2))
plotGenesInEachBatch(data=log10(normalized+1), genes=c("Acsm4", "Ncam2", "Nqo1"), x=sel_positions, batches=c("LML1", "LML2", "LML3"), batchColors=c("red", "blue", "green"), xlab="position LML", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))
```
...
#heatmap.2(HVGs, xlab="Position LML", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, key.xlab = "log10geneExp", Colv = F)
```
...
#HVGs<-log10(getHVGs(normalized[,grep("LML1", colnames(normalized))])[1:1000,]+1)
normHVGs<-normalized[rownames(HVGs),]
#locfit(norm_degs[,gene]~lp(norm_degs[,1], deg = 2, nn=1), data=norm_degs)

tmp3<-batchSubset(data=normHVGs, batch="LML3", positions=sel_positions)
tmp2<-batchSubset(data=normHVGs, batch="LML2", positions=sel_positions)
tmp1<-batchSubset(data=normHVGs, batch="LML1", positions=sel_positions)

#tmp1=shortest dataset
findEqPositions(tmp1, tmp3)[[1]]
boxplot(findEqPositions(tmp1, tmp3)[[3]][,-1], main = "coefficients distribution b3")
#abline(h=mean(findEqPositions(tmp3, tmp2)[[2]]), col="red")
findEqPositions(tmp1, tmp2)[[1]]
boxplot(findEqPositions(tmp1, tmp2)[[3]][,-1], main = "coefficients distribution b2")
...
normalized<-cbind(normalized[,colnames(normalized) %in% paste("LML1", 1:58, sep="-")], normalized[,colnames(normalized) %in% paste("LML2", findEqPositions(tmp1, tmp2)[[1]][1]:findEqPositions(tmp1, tmp2)[[1]][2], sep="-")], normalized[,colnames(normalized) %in% paste("LML3", findEqPositions(tmp1, tmp3)[[1]][1]:findEqPositions(tmp1, tmp3)[[1]][2], sep="-")])

#normalized<-cbind(normalized[,colnames(normalized) %in% paste("LML1", 1:58, sep="-")], normalized[,colnames(normalized) %in% paste("LML2", findEqPositions(tmp1, tmp2)[[1]][1]:findEqPositions(tmp1, tmp2)[[1]][2], sep="-")], normalized[,colnames(normalized) %in% paste("LML3", 4:61, sep="-")])

#order normalized by position

normalized["sel_positions",]<-c(as.numeric(gsub("LML1-", "", colnames(normalized)[grep("LML1", colnames(normalized))])), as.numeric(gsub("LML2-", "", colnames(normalized)[grep("LML2", colnames(normalized))]))-(findEqPositions(tmp1, tmp2)[[1]][1]-1), as.numeric(gsub("LML3-", "", colnames(normalized)[grep("LML3", colnames(normalized))]))-(findEqPositions(tmp1, tmp3)[[1]][1]-1))

#normalized["sel_positions",]<-c(as.numeric(gsub("LML1-", "", colnames(normalized)[grep("LML1", colnames(normalized))])), as.numeric(gsub("LML2-", "", colnames(normalized)[grep("LML2", colnames(normalized))]))-(findEqPositions(tmp1, tmp2)[[1]][1]-1), as.numeric(gsub("LML3-", "", colnames(normalized)[grep("LML3", colnames(normalized))]))-(4-1))
...
batch<-rep("LML1", dim(normalized)[2])
batch[grep("LML2", colnames(normalized))]<-"LML2"
batch[grep("LML3", colnames(normalized))]<-"LML3"

batch_col<-rep("red", length(batch))
batch_col[which(batch=="LML2")]<-"blue"
batch_col[which(batch=="LML3")]<-"green"

AvPerGene<-t(apply(normalized[-dim(normalized)[1],], 1, function(i){tapply(i, batch, mean)}))

normalized[-dim(normalized)[1],which(batch=="LML1")]<-apply(normalized[-dim(normalized)[1],which(batch=="LML1")], 2, function(x){x*rowMeans(AvPerGene)/AvPerGene[,"LML1"]})
normalized[-dim(normalized)[1],which(batch=="LML2")]<-apply(normalized[-dim(normalized)[1],which(batch=="LML2")], 2, function(x){x*rowMeans(AvPerGene)/AvPerGene[,"LML2"]})
normalized[-dim(normalized)[1],which(batch=="LML3")]<-apply(normalized[-dim(normalized)[1],which(batch=="LML3")], 2, function(x){x*rowMeans(AvPerGene)/AvPerGene[,"LML3"]})
...
col=colorRampPalette(c("lightblue", "darkblue"))
plot(samples_pcomp2$x[,1], samples_pcomp2$x[,2], col=alpha(col(96)[sel_positions], 0.3), xlab="PC1", ylab="PC2", pch=19, cex=1)
legend("bottomleft", c("Lateral", "Medial"), pch=19, col=alpha(c("lightblue", "darkblue"), 0.3), cex=0.8)

#See if PC 1 and 2 correlate with position

par(mfrow=c(1, 2), mar=c(4, 4, 4, 2))

plotXYcorrelation(x=samples_pcomp2$x[,1], y=normalized["sel_positions",], main="PC1 vs LM position", ylab="LM position", xlab="PC1", col=batch_col)

plotXYcorrelation(x=samples_pcomp2$x[,2], y=normalized["sel_positions",], main="PC2 vs LM position", ylab="LM position", xlab="PC2", col=batch_col)
```
...
plotGenesInEachBatch(data=log10(normalized+1), genes=c("Cnga2", "Epas1", "Ric8b", "Rtp1", "Rtp2", "Cnga4", "Gap43", "Omp", "Gng13", "Gnal", "Gnb1", "Gnai2", "Gnao1", "Six1", "Alcam", "Ano2", "Adcy3", "Chl1", "Trp63", "Slc2a3", "Neurog1", "Plcb2", "Ascl1", "Trpc2", "Reep1", "Krt5", "Sema3a", "Nrp1", "Olfr1507", "Olfr449"), x=normalized["sel_positions",], batches=c("LML1", "LML2", "LML3"), batchColors=c("red", "blue", "green"), xlab="position LML", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))

par(mfrow=c(2, 2))

plotGenesInEachBatch(data=log10(normalized+1), genes=c("Acsm4", "Ncam2", "Nqo1"), x=normalized["sel_positions",], batches=c("LML1", "LML2", "LML3"), batchColors=c("red", "blue", "green"), xlab="position LML", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))

#PC1 biggest loadings
par(mfrow=c(2, 2))

plotGenesInEachBatch(data=log10(normalized+1), genes=rownames(samples_pcomp2$rotation[order(-abs(samples_pcomp2$rotation[,1])),])[1:10], x=normalized["sel_positions",], batches=c("LML1", "LML2", "LML3"), batchColors=c("red", "blue", "green"), xlab="position LML", ylab="log10 rpm expression", ylim=c(0, max(log10(normalized+1))))
...
#position<-as.numeric(gsub("LML..", "", position))
...
#write.csv(rbind(HVGs, sel_positions=normalized["sel_positions",]), "./supp_tables/LMLlog10HVGs.csv")
...
#pdf(file = "./figures_pdf2/Fig1B_LML.pdf", width=5, height = 5)
...
pdf(file = "./figures_pdf2/Fig1B_alt_LML.pdf", width=5, height = 5)
boxplot(unlist(cor_list), ylab="rho", xlab="LML axis", ylim=c(0, 1))
boxplot(cor_list, ylab="rho", xlab="LML position", ylim=c(0, 1), cex.lab=0.6, cex.axis=0.6)
...
plotMeanDetectedGenesPerPosition(data=normalized[-which(rownames(normalized)=="sel_positions"),], thresh=10, positions=normalized["sel_positions",], errorBars="SDV", xlab="position LML")

normORs<-rbind(normalized[grep("Olfr", rownames(normalized)),], sel_positions=normalized["sel_positions",])
plotMeanDetectedGenesPerPosition(data=normORs[-which(rownames(normORs)=="sel_positions"),], thresh=10, positions=normORs["sel_positions",], errorBars="SDV", xlab="position LML")
rm(normORs)
```
...
#write.csv(normalized, "./newLMLnormalized_rpm_repsAvg_G99.csv")
...
for(marker in c("Cnga2", "Epas1", "Ric8b", "Rtp1", "Rtp2", "Cnga4", "Gap43", "Omp", "Gng13", "Gnal", "Gnb1", "Gnai2", "Gnao1", "Six1", "Alcam", "Ano2", "Adcy3", "Chl1", "Trp63", "Slc2a3", "Neurog1", "Plcb2", "Ascl1", "Trpc2", "Reep1", "Krt5", "Sema3a", "Nrp1", "Olfr1507", "Olfr449")){
  ...
  plot(1:dim(normalized)[2], log10(normalized[marker,]+1), xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position LML", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normalized+1))))
}
...
#pdf(file = "./figures_pdf2/Fig1E_LML.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1", "Reg3g", "Olfr878", "Olfr855", "Olfr449", "Olfr1310", "Olfr1277", "Nrp2")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position LML", ylab="mean rpm expression", pch=19, ylim=c(0, max(normalized[marker,])+1))
  lines(locfit(normalized[marker,]~lp(1:dim(normalized)[2], deg = 2, nn=1), data=as.data.frame(normalized)), col="red")
}
#dev.off()

#write.csv(normalized[c("Acsm4", "Ncam2", "Nqo1", "Reg3g"),], "./supp_tables/spatialMarkersLML.csv")

cairo_ps("./figures_pdf2/Fig1E_LML.eps", width = 5, height = 5)
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position LML", ylab="mean rpm expression", pch=19, ylim=c(0, max(normalized[marker,])+1))
...
```{r}
mOSNpropsNusvr<-read.csv("./supp_tables/LML_mOSNproportions_NuSVR.csv")
mOSNpropsNnls<-read.csv("./supp_tables/LML_mOSNproportions_nnls.csv")

pdf(file = "./figures_pdf2/Fig3A_LML.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in c("mOSNs", "others")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(mOSNpropsNusvr$X+1, mOSNpropsNusvr[,cell], xlim=c(0, length(mOSNpropsNusvr$X)+1), main=paste("NuSVR", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, 1))
  lines(locfit(mOSNpropsNusvr[,cell]~lp(mOSNpropsNusvr$X+1, deg = 2, nn=1), data=as.data.frame(mOSNpropsNusvr)), col="red")
  
  plot(mOSNpropsNnls$X+1, mOSNpropsNnls[,cell], xlim=c(0, length(mOSNpropsNnls$X)+1), main=paste("nnls", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, 1))
...
```{r}
mOSNpropsNusvr<-read.csv("./supp_tables/LML_NeuronsProportions_NuSVR.csv")
mOSNpropsNnls<-read.csv("./supp_tables/LML_NeuronsProportions_nnls.csv")

pdf(file = "./figures_pdf2/Fig3A.2_LML.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(cell in c("neurons", "otherCells")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(mOSNpropsNusvr$X+1, mOSNpropsNusvr[,cell], xlim=c(0, length(mOSNpropsNusvr$X)+1), main=paste("NuSVR", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, 1), col=alpha(1, 0.3))
  lines(locfit(mOSNpropsNusvr[,cell]~lp(mOSNpropsNusvr$X+1, deg = 2, nn=1), data=as.data.frame(mOSNpropsNusvr)), col="red")
  
  plot(mOSNpropsNnls$X+1, mOSNpropsNnls[,cell], xlim=c(0, length(mOSNpropsNnls$X)+1), main=paste("nnls", cell), xlab="position LML", ylab="proportion", pch=19, ylim=c(0, 1), col=alpha(1, 0.3))
  lines(locfit(mOSNpropsNnls[,cell]~lp(mOSNpropsNnls$X+1, deg = 2, nn=1), data=as.data.frame(mOSNpropsNnls)), col="red")
}
dev.off()

autocorrelationTest(filter = c(1, 1), data=t(mOSNpropsNnls[1:28,-1]), maxLag = 28-10)

autocorrelationTest(filter = c(1, 1), data=t(mOSNpropsNusvr[1:28,-1]), maxLag = 28-10)
```
...
pdf(file = "./figures_pdf2/FigS1E_LML_alt.pdf", width=5, height = 5)
heatmap.2(as.matrix(marker_heat), xlab="", trace='none', key.title=NA, key.ylab=NA, col=viridis(length(seq(0,6,0.1))-1), cexRow = 0.8, key.xlab = "log10geneExp", labCol = FALSE, breaks=seq(0,6,0.1), Rowv=F)
dev.off()
...
pdf(file = "./figures_pdf2/Fig1D_LML.pdf", width=5, height = 5)
heatmap.2(as.matrix(marker_heat), xlab="Position LML", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, cexCol=0.5, key.xlab = "log10MeanRPM", Colv = F, Rowv=F)
dev.off()

#write.csv(marker_heat, "./supp_tables/log10MOEMarkersLML.csv")

```

**Heatmap of genes having averaged replicates**

```{r LMLsymmetry, echo=FALSE, message=FALSE, warning=FALSE}

which(normalized["Omp",11:40]==min(normalized["Omp",11:40]))
which(normalized["Gnal",11:40]==min(normalized["Gnal",11:40]))
which(normalized["Cnga2",11:40]==min(normalized["Cnga2",11:40]))

boxplot(log10(normalized[c("Omp", "Cnga2", "Gnal"),]+1), xlab="Position LML", ylab="mean log10 mean rpm expression")
mediumPos=which(colMeans(log10(normalized[c("Omp", "Cnga2", "Gnal"),]+1))==min(colMeans(log10(normalized[c("Omp", "Cnga2", "Gnal"),]+1))[15:45]))


```
**Mean log10 expression of Omp, Gnal and Cnga2**
Medium position will be where the bone is, so where neuronal markers drop

```{r, LMLsymmetry2, echo=FALSE, message=FALSE, warning=FALSE}
###

minExpThrs<-apply(HVGsAvgs, 1, function(x){length(which(x>50))})
HVGsAvgs_red<-HVGsAvgs[which(minExpThrs>=3),]
#HVGsAvgs_red<-HVGsAvgs

corsLMLhalves<-apply(HVGsAvgs_red, 1, function(x){cor(x[1:28], x[56:29], method="spearman")})
corsLMLhalvesP<-apply(HVGsAvgs_red, 1, function(x){cor.test(x[1:28], x[56:29], method="spearman")$p.value})
boxplot(corsLMLhalves, ylim=c(-1, 1), main="correlation between LML axis halves in HVGs")
boxplot(corsLMLhalvesP, ylim=c(min(corsLMLhalvesP), max(corsLMLhalvesP)))

#Average positions 1:28 and 56:29
new<-t(apply(normalized, 1, function(x){c(rowMeans(data.frame(x[1:mediumPos], x[rev((mediumPos+1):(mediumPos*2))])), rev(rowMeans(data.frame(x[1:mediumPos], x[rev((mediumPos+1):(mediumPos*2))]))))}))

new[dim(new)[1],]<-1:dim(new)[2]
colnames(new)<-1:dim(new)[2]

#just take half of the data (lateral - middle)
new<-new[,1:(dim(new)[2]/2)]

par(mfrow=c(2, 2))
plot(log10(new["Omp",]+1), main="Omp", ylim=c(0, max(log10(new+1))), xlab="Position LM", ylab="log10 mean rpm expression")
plot(log10(new["Cnga2",]+1), main="Cnga2", ylim=c(0, max(log10(new+1))), xlab="Position LM", ylab="log10 mean rpm expression")
plot(log10(new["Gnal",]+1), main="Gnal", ylim=c(0, max(log10(new+1))), xlab="Position LM", ylab="log10 mean rpm expression")
normalized=new
#x<-data.frame(as.numeric(normalized["Omp",1:mediumPos]), as.numeric(normalized["Omp",rev((mediumPos+1):(mediumPos*2))]))

#newVals<-c(rowMeans(x), rev(rowMeans(x)))
#plot(newVals, main="Omp")

```

**Symmetric expression of neuronal markers**

```{r OSN_markers6uyi, echo=FALSE, message=FALSE, warning=FALSE}
agGenes<-read.csv("./supp_tables/ag_genes4deconv_T2.csv")
pdf(file = "./figures_pdf2/Fig1E_LM_agOlfrs_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(marker in agGenes$genes[grep("Olfr", agGenes$genes)][agGenes$genes[grep("Olfr", agGenes$genes)] %in% rownames(normalized)]){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position LM", ylab="mean rpm expression", pch=19, ylim=c(0, max(normalized[marker,])+1))
  lines(locfit(normalized[marker,]~lp(1:dim(normalized)[2], deg = 2, nn=1), data=as.data.frame(normalized)), col="red")
}
dev.off()
...
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  plot(1:dim(normalized)[2], log10(normalized[marker,]+1), xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position LM", ylab="log10 mean rpm expression", pch=19,  ylim=c(0, max(log10(normalized+1))))
}
...
pdf(file = "./figures_pdf2/Fig1E_LML_QCcheck_T3_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
...
pdf(file = "./figures_pdf2/Fig1E_LML_rev.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
...
pdf(file = "./figures_pdf2/NeuroMksHM_LM_rev.pdf", width=7, height = 7)
heatmap.2(as.matrix(heatNeuroMks), Colv = NA, xlab="Position LM", trace='none', key.xlab="log10meanRPMnorm", key.ylab=NA, key.title=NA, col=viridis(100), cexRow = 0.8, cexCol = 0.8)
dev.off()
```

**Symmetric markers expression (after averaging replicates)**
...
pdf(file = "./figures_pdf2/Fig3B_LML.pdf", width=5, height = 5)
par(mfrow=c(2, 2))
for(marker in geneList$gene){
  #plot(1:dim(normalized)[2], log10(normalized[marker,]+1), xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position LM", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normalized[geneList$gene,]+1))))
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position LM", ylab="log10 mean rpm expression", pch=19)
  lines(locfit(normalized[marker,]~lp(1:dim(normalized)[2], deg = 2, nn=1), data=as.data.frame(normalized)), col="red")
}
dev.off()

write.csv(normalized[c("Fgf20", "Wfdc18", "Rps21"),], "./supp_tables/someGenesFromOtherCells_LML.csv")

cairo_ps("./figures_pdf2/Fig3B_LML.eps", width = 5, height = 5)
par(mfrow=c(2, 2))
for(marker in geneList$gene){
  #plot(1:dim(normalized)[2], log10(normalized[marker,]+1), xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position LM", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normalized[geneList$gene,]+1))))
  plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=paste(marker, geneList$cellType[which(geneList$gene==marker)]), xlab="position LM", ylab="log10 mean rpm expression", pch=19)
...
write.csv(normalized, "./supp_tables/newLML_ORS_normalized_rpm_repsAvg_mks_G99.csv")
```
...
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  plot(1:dim(normCanonMks)[2], log10(normCanonMks[marker,]+1), xlim=c(0, dim(normCanonMks)[2]+1), main=marker, xlab="position LM", ylab="log10 mean rpm expression", pch=19, ylim=c(0, max(log10(normCanonMks+1))))
}

par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Ncam2", "Nqo1")){
  plot(1:dim(normCanonMks)[2], normCanonMks[marker,], xlim=c(0, dim(normCanonMks)[2]+1), main=marker, xlab="position LM", ylab="mean rpm expression", pch=19, ylim=c(0, max(normCanonMks[marker,])+1))
}
```
...
heatmap.2(as.matrix(marker_heat), xlab="Position LM", trace='none', key.title=NA, key.ylab=NA, col=viridis(100), cexRow = 0.8, key.xlab = "log10MeanRPM", Colv = F)
...
col=colorRampPalette(c("lightblue", "darkblue"))
plot(samples_pcompAvgs$x[,1], samples_pcompAvgs$x[,2], col=alpha(col(30)[1:dim(norm_complete)[2]], 0.3), xlab="PC1", ylab="PC2", pch=19, cex=1)
legend("bottomleft", c("Lateral", "Medial"), pch=19, col=alpha(c("lightblue", "darkblue"), 0.3), cex=0.8)
...
plotXYcorrelation(x=samples_pcompAvgs$x[,1], y=1:dim(norm_complete)[2], main="PC1 vs LM position", ylab="LM position", xlab="PC1", col=1)

plotXYcorrelation(x=samples_pcompAvgs$x[,2], y=1:dim(norm_complete)[2], main="PC2 vs LM position", ylab="LM position", xlab="PC2", col=1)
```
...
col=colorRampPalette(c("lightblue", "darkblue"))
plot(samples_pcompAvgs$x[,1], samples_pcompAvgs$x[,2], col=alpha(col(30)[1:dim(normalized2)[2]], 0.3), xlab="PC1", ylab="PC2", pch=19, cex=1)
legend("bottomleft", c("Lateral", "Medial"), pch=19, col=alpha(c("lightblue", "darkblue"), 0.3), cex=0.8)
...
plotXYcorrelation(x=samples_pcompAvgs$x[,1], y=1:dim(normalized2)[2], main="PC1 vs LM position", ylab="LM position", xlab="PC1", col=1)

plotXYcorrelation(x=samples_pcompAvgs$x[,2], y=1:dim(normalized2)[2], main="PC2 vs LM position", ylab="LM position", xlab="PC2", col=1)
...
LMdists<-as.matrix(dist(1:dim(HVGsAvgs)[2]))
LMdists.inv<-1/LMdists
diag(LMdists.inv)<-0

moransIs<-sapply(1:dim(HVGsAvgs)[1], function(x){Moran.I(HVGsAvgs[x,], LMdists.inv)$observed})

moransIsExp<-sapply(1:dim(HVGsAvgs)[1], function(x){Moran.I(HVGsAvgs[x,], LMdists.inv)$expected})

moransIsPvals<-sapply(1:dim(HVGsAvgs)[1], function(x){Moran.I(HVGsAvgs[x,], LMdists.inv)$p.value})
...
tmp<-data.frame(LMmoransIs=moransIs, LMmoransIsPvals=moransIsPvals)
rownames(tmp)<-rownames(HVGsAvgs)
write.csv(tmp, "./supp_tables/LMmoransI.csv")

```

##Differential expression along LML axis

In order to find genes which are differentially expressed across the LML axis, I performed an autocorrelation test. So if a gene "g"'s expression at positions "x" correlated significantly (p<0.05) with gene expression at positions ("x+1"); and the correlation coefficient was bigger than 0.4, g is differentially expressed.
...
#heatmap.2(as.matrix(heatDEGs2.1), Colv = NA, xlab="Position LML", trace='none', key.xlab="normalized gene expression", key.ylab=NA, key.title=NA, col=viridis(100), cexRow = 0.8, cexCol = 0.8, breaks=seq(0,1,0.1))

heatmap.2(as.matrix(heatDEGs2.1), Colv = NA, xlab="Position LM", trace='none', key.xlab="log10MeanRPMnorm", key.ylab=NA, key.title=NA, col=viridis(100), cexRow = 0.8, cexCol = 0.8)
```
...
write.csv(DEGsfile, "./supp_tables/newLMLDEORs.autocor.binMatFDR0.01filt_NormMks_G99.csv")
...
#write.csv(dframe, "./clusters_deORsLML.csv")
```
...
for(i in (1:length(c1NoDE))){
  plot(position, log10(normalized[which(rownames(normalized)==c1NoDE[i]),]+1), main=c1NoDE[i], xlab="position LM", ylab="log10 mean rpm expression", pch=19)
...
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORs, ylim=c(0, 1), ylab="fitted norm. log10 mean rpm expression", xlab="position LM")

```

**Figure 16.** Average gene expression per cluster across the LML axis and spatial expression pattern per gene in colors. Gene expression values were log10 scaled and then normalized to values between 0 and 1.
...
#  par(mfrow=c(2, 2))
#  for(marker in genes){
#    plot(1:dim(normalized)[2], normalized[marker,], xlim=c(0, dim(normalized)[2]+1), main=marker, xlab="position LML", ylab="rpm expression", pch=19, col=alpha(cluster, 0.3))
#  }
#}
...
write.csv(normalized, "./supp_tables/newLML_AllGenes_normalized_rpm_repsAvg_mks.csv")
...
plot(position, log10(normalized["Acsm4",]+1), xlim=c(0, max(position)+1), main="Acsm4", xlab="position LM", ylab="log10 mean rpm expression", pch=19)
abline(h=median(log10(normalized["Acsm4",]+1)), col="red")


plot(position, binMat["Acsm4",], xlim=c(0, max(position)+1), main="Acsm4", xlab="position LM", ylab="binarized expression", pch=19)
...
extraDEGs<-read.csv("./3D/LMLextraDEGs.csv", stringsAsFactors = F, row.names=1)
top20extraDEGs<-genes.table[which(genes.table$genes.names %in% extraDEGs$x),]
top20extraDEGs<-top20extraDEGs[1:20,]

write.csv(top20extraDEGs, "./supp_tables/top20LMLextraDEGs.csv")
```
...
write.csv(DEGsfile, "./supp_tables/newLMLDEGs.autocor.binMatFDR0.01filt_G99.csv")
...
write.csv(truePatts_table, "./supp_tables/SDEtest_DEGsTruePatts_LM.csv")
...
pdf("./figures_pdf2/mks_CTtrend_LM_rev.pdf")
par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Nqo1")){
  #plot(1:78, normalized[marker,], xlim=c(0, 80), main=marker, xlab="position P-A", ylab="rpm expression")
  plot(1:dim(DEGs2test)[2], DEGs2test[marker,], xlim=c(0, dim(DEGs2test)[2]+1), main=marker, xlab="position LM", ylab="mean rpm expression", pch=19, ylim=c(0, max(DEGs2test[marker,])+1))
  lines(locfit(DEGs2test[marker,]~lp(1:dim(DEGs2test)[2], deg = 2, nn=1), data=as.data.frame(DEGs2test)), col="red")
}

par(mfrow=c(2, 2))
for(marker in c("Acsm4", "Nqo1")){
  plot(1:dim(geneExpPat_due2CTdist)[2], geneExpPat_due2CTdist[marker,], xlim=c(0, dim(geneExpPat_due2CTdist)[2]+1), main=paste(marker, "CT pattern"), xlab="position LM", ylab="mean rpm expression", pch=19, ylim=c(0, max(geneExpPat_due2CTdist[marker,])+1))
...
#par(mfrow=c(2, 2))
#for(marker in c("Acsm4", "Nqo1")){
#  plot(1:dim(trueExpPatterns)[2], trueExpPatterns[marker,], xlim=c(0, dim(trueExpPatterns)[2]+1), main=marker, xlab="position LM", ylab="mean rpm expression", pch=19, ylim=c(min(trueExpPatterns[marker,]), max(trueExpPatterns[marker,])+1))
...
write.csv(data.frame(geneName=names(spearmanFDRs), spearmanRho=spearmanCors, pval=spearmanPvals, fdr=spearmanFDRs, is.predictable=is.predictable), "./supp_tables/DEGsCTpredictability_LM_rev.csv")

write.csv(geneExpPat_due2CTdist, "./supp_tables/DEGsExpPat_due2CTdist_LM.csv")
#write.csv(MeanGeneExpPerCT, "./supp_tables/MeanGeneExpPerCT_SC.csv")

#pdf("./figures_pdf2/predictableOlfrs_LM_rev.pdf")
#par(mfrow=c(2, 2))
#for(marker in CTpredictableDEGs[grep("Olfr", CTpredictableDEGs)]){
  
  #plot(1:dim(DEGs2test)[2], DEGs2test[marker,], xlim=c(0, dim(DEGs2test)[2]+1), main=paste(marker, "original pattern"), xlab="position LM", ylab="mean rpm expression", pch=19, ylim=c(0, max(DEGs2test[marker,])+1))
  #lines(locfit(DEGs2test[marker,]~lp(1:dim(DEGs2test)[2], deg = 2, nn=1), data=as.data.frame(DEGs2test)), col="red")
#}

#par(mfrow=c(2, 2))
#for(marker in CTpredictableDEGs[grep("Olfr", CTpredictableDEGs)]){
  #plot(1:dim(geneExpPat_due2CTdist)[2], geneExpPat_due2CTdist[marker,], xlim=c(0, dim(geneExpPat_due2CTdist)[2]+1), main=paste(marker, "pattern due to cell types dist."), xlab="position LM", ylab="mean rpm expression", pch=19, ylim=c(0, max(geneExpPat_due2CTdist[marker,])+1))
  #lines(locfit(geneExpPat_due2CTdist[marker,]~lp(1:dim(geneExpPat_due2CTdist)[2], deg = 2, nn=1), data=as.data.frame(geneExpPat_due2CTdist)), col="red")
#}
#dev.off()
...
par(mfrow=c(3, 3))
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORsMinCS100, ylim=c(0, 1), ylab="fitted norm. log10 mean expression", xlab="position LM")
...
par(mfrow=c(1, 1))
plot(x=cluster2index$myIndx, y=cluster2index$zone.index, col=alpha(cluster2index$Cluster, 0.3), xlab="our LM Indexes", ylab="Zone index by Miyamichi", pch=19)
...
par(mfrow=c(3, 3))
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORsMinCS25, ylim=c(0, 1), ylab="fitted norm. log10 mean expression", xlab="position LM")
...
write.csv(nGenesPerClust, "./supp_tables/nGenesPerClust_LML.csv")

ORsUmap<-umap(deORs) #normalized Differentially Expressed OR genes
pdf(file = "./figures_pdf2/Fig2D.1.pdf", width=5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(clusters_deORs, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

cairo_ps("./figures_pdf2/Fig2D.1.eps", width = 5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(clusters_deORs, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

pdf(file = "./figures_pdf2/FigS4A_LML.1.pdf", width=5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(ORcols, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

cairo_ps("./figures_pdf2/FigS4A_LML.1.eps", width = 5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(ORcols, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
dev.off()

plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(zoneCol, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")

pdf(file = "./figures_pdf2/FigS4A_LML.2.pdf", width=5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(classCol, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
legend("topright", c("Class 1", "Class 2"), pch=19, col=alpha(c("red", "blue"), 0.3))
dev.off()

cairo_ps("./figures_pdf2/FigS4A_LML.2.eps", width = 5, height = 5)
plot(ORsUmap$layout[,1], ORsUmap$layout[,2], col=alpha(classCol, 0.3), pch=19, xlab="UMAP1", ylab="UMAP2")
legend("topright", c("Class 1", "Class 2"), pch=19, col=alpha(c("red", "blue"), 0.3))
dev.off()
```
...
dpt <- DPT(ORsDiffmap, tips=which(names(clusters_deORs)=="Olfr399")) #pseudotime
...
#identify first gene in DCs projection from brown cluster
#clusters_deORs[which(projection$s[,1]==min(projection$s[,1][which(clusters_deORs=="brown" & projection$s[,1]<0 & projection$s[,2]<(-0.04))]))]
#Prss33

#most medial Zone 1 OR in Miyamichi paper
#cluster2index[cluster2index$myIndx==28,]
#Olfr399
```
...
#Expression patterns per cluster
pdf(file = "./figures_pdf2/Fig2D.2.pdf", width=5, height = 5)
par(mfrow=c(3, 3))
plotMeanPatternPerCluster(data=deORs, clusters=clusters_deORs, ylim=c(0, 1), ylab="fitted norm. log10 mean expression", xlab="position LM")
dev.off()
...
pdf(file = "./figures_pdf2/FigS4A_LML.3.pdf", width=5, height = 5)
plot(ORsDiffmap$DC1, ORsDiffmap$DC2, col=alpha(plot.DPT(dpt)$data$ColourExpl, 0.3), pch=19, xlab="DC1", ylab="DC2")
dev.off()

cairo_ps("./figures_pdf2/FigS4A_LML.3.eps", width = 5, height = 5)
plot(ORsDiffmap$DC1, ORsDiffmap$DC2, col=alpha(plot.DPT(dpt)$data$ColourExpl, 0.3), pch=19, xlab="DC1", ylab="DC2")
dev.off()
...
pdf(file = "./figures_pdf2/FigS4A_LML.4.pdf", width=5, height = 5)
plot(x=cluster2dpt$myIndx, y=cluster2dpt$zone.index, col=alpha(cluster2dpt$Cluster, 0.3), xlab="Indexes by DPT", ylab="Zone index by Miyamichi", pch=19)
dev.off()

cairo_ps("./figures_pdf2/FigS4A_LML.4.eps", width = 5, height = 5)
plot(x=cluster2dpt$myIndx, y=cluster2dpt$zone.index, col=alpha(cluster2dpt$Cluster, 0.3), xlab="Indexes by DPT", ylab="Zone index by Miyamichi", pch=19)
dev.off()
...
DEGsfile<-read.csv("./supp_tables/newLMLDEGs.autocor.binMatFDR0.01filt_G99.csv")[,-1]
...
is.LMLdeg<-sapply(neuroActMks_Up, function(x){x %in% DEGsfile$Gene.name})
LMLclust<-as.character(sapply(neuroActMks_Up, function(x){DEGsfile$clustersMinCS50[which(DEGsfile$Gene.name==x)]}))

neuroActMks_Up<-data.frame(neuroActMks_Up, is.LMLdeg, LMLclust)
write.csv(neuroActMks_Up, "./supp_tables/neuroActMks_Up_LML.csv")

neuroActMks_Down<-c("Bace1", "Calb2", "Cdh15", "Chil3", "Efna5", "Entpd2", "Etv3", "Kirrel3", "Ppp3ca", "Rims3")
is.LMLdeg<-sapply(neuroActMks_Down, function(x){x %in% DEGsfile$Gene.name})
LMLclust<-as.character(sapply(neuroActMks_Down, function(x){DEGsfile$clustersMinCS50[which(DEGsfile$Gene.name==x)]}))

neuroActMks_Down<-data.frame(neuroActMks_Down, is.LMLdeg, LMLclust)
write.csv(neuroActMks_Down, "./supp_tables/neuroActMks_Down_LML.csv")
```
...
plotGenesFromCluster(data=deORs_data, cluster="brown", clustersColName="clusters", chromosomesColName="Chromosome.scaffold.name", startPosColName="Gene.start..bp.", endPosColName="Gene.end..bp.", genome="mm10", main="Black cluster ORs positions in chromosomes")
...
pdf(file = "./figures_pdf2/FigS4C_LML.pdf", width=5, height = 5)
plot(meansLog10GenDist, meansLog10PattDist, ylim=c(0, max(meansLog10PattDist)+100), xlim=c(0, 8.5), pch=19, xlab="log10(Genomic Distance)", ylab="Pattern Distance")
...
cairo_ps("./figures_pdf2/FigS4C_LML.eps", width = 5, height = 5)
plot(meansLog10GenDist, meansLog10PattDist, ylim=c(0, max(meansLog10PattDist)+100), xlim=c(0, 8.5), pch=19, xlab="log10(Genomic Distance)", ylab="Pattern Distance")
arrows(meansLog10GenDist, meansLog10PattDist-sdLog10PattDist, meansLog10GenDist, meansLog10PattDist+sdLog10PattDist, length=0.05, angle=90, code=3)
set.seed(10)
abline(h=mean(sample(PatternDist, 500), na.rm=T))
...
plotXYcorrelation(x=real, y=predictedRandomTraining, xlab = "real indexes", ylab="predicted indexes", col=1, main="RF predictions for OR LM indexes")
par(mar=c(8, 5, 5, 5))
...
cor(ORs_dataRF$DPTindex, MiyIndex, method="spearman", use="complete.obs")
...
for(cluster in unique(DEGsfile$clustersMinCS25)){
  ...
  write.table(tmp, paste("./supp_tables/forGOanalysis/", cluster, "_newLMDEGs_minCS25.csv", sep=""), row.names = F, quote = F, col.names = F)
}
...
for(cluster in unique(DEGsfile$clustersMinCS50)){
  ...
  write.table(tmp, paste("./supp_tables/forGOanalysis/", cluster, "_newLMDEGs_minCS50.csv", sep=""), row.names = F, quote = F, col.names = F)
}

for(cluster in unique(DEGsfile$clustersMinCS100)){
  tmp=DEGsfile$Gene.name[which(DEGsfile$clustersMinCS100==cluster)]
  write.table(tmp, paste("./supp_tables/forGOanalysis/", cluster, "_newLMDEGs_minCS100.csv", sep=""), row.names = F, quote = F, col.names = F)
}

tmp=rownames(normalized)[which(filt==1)]
write.table(tmp, "./supp_tables/forGOanalysis/newLMLgenes_GObg_ORs.csv", row.names = F, quote = F, col.names = F)

tmp=rownames(normalized)[which(filt==1)]
tmp=tmp[-grep("Olfr", tmp)]
write.table(tmp, "./supp_tables/forGOanalysis/newLMgenes_GObg.csv", row.names = F, quote = F, col.names = F)
```

```{r GOplotsMinCS50, echo=FALSE, message=FALSE, warning=FALSE}

pdf(file = "./figures_pdf2/FigS2A_LM.pdf", width=7, height = 5)

par(las=2, mar=c(6,21,4,3))

for(file in paste("./supp_tables/forGOanalysis/gorilla_GO/", list.files(path="./supp_tables/forGOanalysis/gorilla_GO/", pattern = "GO_LM.+MinCS50"), sep="")){
  ...
  plotTitle<-gsub("./supp_tables/forGOanalysis/gorilla_GO/GO_LM", "", file)
...
#for(file in paste("./supp_tables/forGOanalysis/gorilla_GO/", list.files(path="./supp_tables/forGOanalysis/gorilla_GO/", pattern = "GO_LM.+MinCS100"), sep="")){
#  GSEA<-read.csv(file, stringsAsFactors = F, sep="\t")
#  GSEA<-GSEA[grep("GO:", GSEA$GO.Term),1:5]
#  GSEA<-GSEA[which(GSEA$FDR.q.value<0.05),]
#  plotTitle<-gsub("./supp_tables/forGOanalysis/gorilla_GO/GO_LM", "", file)
...
write.csv(normalized, "./supp_tables/newLML_nonORS_normalized_rpm_repsAvg_G99.csv")
...
write.csv(DEGsfile, "./supp_tables/newLMLDEnonORs.autocor.binMatFDR0.01filt_G99.csv")
